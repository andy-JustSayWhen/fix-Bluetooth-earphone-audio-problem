...
关联规格：[`如何判定蓝牙音频设备的音频模式.md`](如何判定蓝牙音频设备的音频模式.md)。恢复流程不得改变该文档规定的模式判定规则。

原因定位技术方案：[`如何排查程序只请求蓝牙输入格式但未开始录音.md`](如何排查程序只请求蓝牙输入格式但未开始录音.md)。该方案用于解释设备为何处于 HFP，不属于当前一键恢复执行步骤。
...
- 目标设备已经断开或输出端点不存在。
- 检测到本机程序正在读取目标设备的麦克风。
- 详细日志能够把触发 HFP 的蓝牙音频格式请求定位到具体程序，并且退出该程序后的同条件对照连续六次恢复高采样率；本项目已登记的实例是微信输入法进程在 2026-07-18 09:14:36.254 提交的格式请求。该请求的上游条件尚未确认，不得仅因微信输入法正在运行就自动结束它。
- 当前主机、输入设备和输出设备组合与实测记录完全一致，并再次出现“选择蓝牙输入 A 后、没有实际采集仍使蓝牙输出 B 降级”的完整证据链。
- 目标设备不是当前默认输出。

### 高度疑似原因

- 麦克风占用刚刚结束，但输出仍不高于 `16 kHz`：疑似通话链路残留。
- 最近系统日志中存在某进程提交 `kBluetoothAudioDevicePropertyFormat request`，当前又未检测到该进程或其他进程正在读取目标麦克风：疑似“程序没有开始录音，但提交了蓝牙输入格式请求”。具体排查方法见 [`如何排查程序只请求蓝牙输入格式但未开始录音.md`](如何排查程序只请求蓝牙输入格式但未开始录音.md)；当前一键恢复不自动执行该日志排查，也不据此自动退出程序。
- 实际采样率已经高于 `16 kHz`，但播放器仍无声，且检测到 SoundSource 等声音路由软件：疑似旧应用输出通道或声音路由残留。
- 目标设备仍是默认输入，但当前未检测到读取者，且没有命中已登记的跨设备确证组合：默认输入只是后续麦克风调用的候选路由，可能增加再次触发通话模式的风险，但不能把“仅设为默认输入”当作已进入 HFP 或已确认占用。
...
### 第 2.5 阶段：释放无占用的 SCO 通话声音连接

当目标设备仍是默认输入和默认输出、未检测到任何程序读取目标麦克风，但输入端点仍处于运行状态且输出不高于 `16 kHz` 时，可以调用系统 `IOBluetoothHandsFree` 提供的 `disconnectSCO`，只请求关闭 SCO 通话声音连接。

该动作必须满足：

1. 调用前再次确认目标麦克风占用列表为空；存在占用时禁止执行。
2. 不改变默认输入、默认输出和系统声音输出设备。
3. 不断开目标设备的蓝牙基础连接，不删除配对记录。
4. 调用返回不代表成功；仍须按本规格连续六次验证实际输出高于 `16 kHz`，并确认目标设备仍同时是默认输入和默认输出。
5. 如果系统没有暴露可控制的免提会话，或调用后 SCO 未关闭，必须记录失败并继续下一层恢复，不得宣称已经恢复。

### 第 2.6 阶段：按正常接入顺序重建蓝牙音频端点

如果正常日志证明设备主动连接时先发布 HFP 端点、后发布 A2DP 端点，并由声音核心在两种端点齐全后明确选择 A2DP，而异常现场是两种端点已同时在线时重启声音核心，则允许执行一次“只断开设备、重启声音核心、等待设备主动重连”的顺序恢复。

1. 只关闭当前基础蓝牙连接，不取消配对，也不由工具立即主动重连。
2. 断开期间重启声音核心，使其不继承两种端点同时在线的旧状态。
3. 等待设备主动重连；若超时未重连，记录本方法失败，不得假报恢复。
4. 最终仍须确认目标设备同时是默认输入和默认输出，并连续六次实际输出高于 `16 kHz`。
