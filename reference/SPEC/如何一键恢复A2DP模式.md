...
- SoundSource 是否运行；
- 目标设备当前是否仍连接。
- 系统当前输出音量与静音状态，供链路恢复后重新同步；不得擅自提高到用户未设置的音量。
...
6. 将目标设备设为默认输出。
7. 执行成功判定。
8. 通过成功判定后，将恢复前保存的输出音量重新同步给 A2DP 端点，再次确认采样率没有回落。同步时只允许在原值附近做 1 个百分点的短暂变化以强制触发绝对音量下发，最终必须回到用户原值。
...
- 输出音量读取与重新同步：`tools/bluetooth-audio-mode-checker/core/macos-audio-volume/`
