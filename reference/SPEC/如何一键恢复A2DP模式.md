...
只要卡片判定为 HFP/HSP，模式胶囊就作为本工作流入口：静止状态保留当前模式文案，鼠标悬浮或键盘聚焦时显示 `一键修复 HFP`。麦克风正在使用时也不得隐藏入口；点击仍需用户确认，并继续遵守本文件的原因定位和处理边界。

点击确认后，页面必须立即进入可见的请求中状态，不得等后端返回后才变化：模式胶囊显示“正在修复，请稍候…”，同一设备不得重复提交修复请求。

本阶段只提供最低限度的状态反馈，不展示虚构百分比或详细阶段时间线。请求完成后，页面必须退出请求中状态并展示结果；接口错误或连接中断也必须显示明确失败提示，不能恢复成看似什么都没发生的初始状态。
...
- 自动测试共 36 项通过，其中 5 项覆盖格式请求解析、`StartIO` 排除、`tsco` 两秒时序、唯一目标约束和反向请求排除，2 项覆盖一键修复忙碌状态、重复提交拦截和结果保留。
...
- 使用正式页面代码和临时返回数据实测：点击后立即显示“正在修复，请稍候…”，请求结束后替换为原因结果；麦克风仍处于占用状态时结果不会被删除。
