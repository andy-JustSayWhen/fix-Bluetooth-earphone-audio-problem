...
4. 未恢复时，重启 `audioaccessoryd（管理声音配件状态和路由协作的系统进程）`，再观察。
5. 未恢复时，断开并重新连接目标蓝牙设备，恢复点击前路由并完成最终观察。
6. 仍未恢复时结束本轮并报告错误，不回到前面的步骤，不重复同一动作。
...
前三步均未恢复时，重启当前用户会话中的 `audioaccessoryd`：

- 当前机器上它由系统启动管理器以 `com.apple.BTServer.cloudpairing` 服务托管；实现必须根据当前用户和实际服务解析目标，不得误伤其他用户会话。
- 记录旧进程号，通过系统服务管理接口请求重启；不得把它当成普通原因进程直接加入前三步的结束名单。
- 最多等待 `5` 秒确认旧进程退出且新进程以不同进程号运行，每 `100` 毫秒检查一次。该 `5` 秒是新步骤的暂定安全上限，真实实现验收后必须用本机样本校准。
- 重启失败、权限不足或新进程未出现时记录失败并继续第五步，不得无限等待。
- 重启后若默认输入或默认输出发生变化，先恢复仍可用的点击前设备，再执行稳定恢复观察。

`coreaudiod（系统核心声音后台进程）`不属于本步骤。重启它会中断全机声音、重建声音硬件抽象层和插件连接，影响范围明显大于重启声音配件服务；除非后续另立规格并经过真实验收，不得把它悄悄加入一键修复。

## 第五步：断开并重新连接

- 每台目标设备每次点击最多执行一次断开重连。
- 连接调用内部时限保持 `18` 秒；超时或报错后也必须继续读取实时设备状态，不得让工作流卡住。
- 调用结束后最多等待 `3` 秒确认目标输入或输出端点重新出现，每 `100` 毫秒检查一次。
- 目标出现后恢复仍可用的点击前默认输入和默认输出；每个方向最多等待 `2` 秒确认切换成功。
- 目标仍未出现时，恢复其他仍可用的点击前路由，并报告“目标设备仍断开，需要手动重连”。
- 恢复路由后执行最终稳定观察；失败即结束本轮，不再回到前面的步骤。
...
- `audioaccessoryd`、`audiomxd（管理应用声音会话的系统进程）`、`bluetoothd（管理蓝牙连接的系统进程）`、`coreaudiod`、系统启动管理器和系统核心进程不得作为普通原因进程结束。
- 第四步只允许按服务身份重启 `audioaccessoryd`；第五步只允许操作当前目标蓝牙设备。
- 任何证据读取失败都必须写入详细日志。证据不足意味着“不结束进程”，不意味着跳过后续安全步骤。
- 每一步成功即停；每一步失败只向后走；禁止回跳、循环复查、重复授权或盲试全部动作。

## 检查清单

- 是否严格执行“实时占用 → 格式请求 → 不同蓝牙设备与切换复位 → 重启 `audioaccessoryd` → 断开重连”。
...
- 是否保留 `2` 秒进程退出、`500` 毫秒链路等待、`1` 秒中转保持、连续三次且间隔 `500` 毫秒的稳定观察、`30` 分钟授权有效期、`10` 秒结果展示和 `18` 秒重连时限。
- `audioaccessoryd` 是否按当前用户的系统服务重启，并确认新旧进程号变化；是否没有误重启 `coreaudiod`。
