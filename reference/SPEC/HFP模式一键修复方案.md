...
“声音输入进程正在运行”不等于“蓝牙麦克风被占用”。仅当进程明确关联到某个实体蓝牙麦克风端点时，才能确认“该进程正在占用该蓝牙麦克风”；不要求该麦克风先出现 `tsco`。设备列表为空、只关联 `AudioTap（系统声音抓取通道）`、内置输入、虚拟输入、回环输入或无法确认蓝牙实体端点的声音输入活动，不得进入蓝牙麦克风占用处理，不得结束进程、请求阻止自动拉起或保留占用授权。

蓝牙麦克风占用处理是所有原因路由之前的前置动作：只要确认任意本机进程正在读取任意实体蓝牙麦克风，就先请求该进程退出，再观察目标设备是否稳定恢复 A2DP。目标已经稳定恢复时立即结束；未恢复时才根据退出后的最新输入、输出、链路和进程现场进入多端点会话、链路残留、格式请求或声音链路重建处理。不得在已确认蓝牙麦克风占用仍存在时先要求用户选择保留输入或输出。
...
3. 实际修复首先检查是否存在“进程明确关联实体蓝牙麦克风端点”的完整证据；只要完整命中，不论当前输入输出是否多端点、该麦克风最新链路是否为 `tsco`，都先结束对应进程并观察目标是否稳定恢复 A2DP。
...
flowchart TD
    ...
    J -->|新快照不再确认同一实体蓝牙麦克风占用| J2[前端撤销过时授权并请求沿用原回合继续]
    J2 --> H
    J -->|用户授权| J0{后端重新确认同一进程仍在读取同一实体蓝牙麦克风}
    J0 -->|否| H
...
- 蓝牙麦克风占用的等待授权只在所列进程仍明确关联同一实体蓝牙麦克风端点时有效，不要求该设备最新链路为 `tsco`。关联消失时，前端必须撤销过时授权并沿用原修复回合继续原因匹配；用户点击授权时后端仍须独立补做一次实时复核，不能仅凭页面保存的等待状态阻止进程。设备列表为空或只发现内置输入、系统声音采集活动时，授权从一开始就不得建立。
...
- 第一优先级是蓝牙麦克风占用前置门槛：进程必须明确关联实体蓝牙麦克风端点；不要求该设备最新链路为 `tsco`。完整命中后先结束对应进程并观察目标是否稳定恢复 A2DP；仅有运行中的声音输入流、进程身份、内置输入或空设备列表均不足。
- 第二优先级是多端点门槛：前置占用处理后目标仍未恢复、当前没有已确认蓝牙麦克风占用、默认输入和默认输出来自两台不同蓝牙设备，并且目标设备最新链路为 `tsco`，才归为多端点会话类。前端风险提示只基于设备组合提前告警，不参与服务端确诊。
- 第三优先级是链路残留门槛：没有已确认蓝牙麦克风占用，但目标设备最新链路为 `tsco`，直接归为链路残留类。此前是否抓到占用进程可以作为案例证据，但不是当前路由的必要条件。
