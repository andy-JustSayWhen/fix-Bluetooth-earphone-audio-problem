...
- 点击一键修复可能使依赖文本框焦点的语音功能立即结束。第一步实时复核已经没有可结束的实体蓝牙麦克风占用进程时，必须保持当前输入输出不变并立即读取一次目标模式：当前不是 A2DP 就直接进入第二步，不等待其进入；当前已经是 A2DP 才连续观察 `3` 秒，期间始终保持 A2DP 便以“点击后自行恢复”结束，否则进入第二步。不得在该节点切换设备、结束格式请求进程或重建声音链路。该保护只位于第一步与第二步之间，不在其他跳过节点重复增加等待。
...
实时复核没有可结束的占用进程时，不得立刻操作声音路由。先保持现场不变并立即读取一次目标模式：当前不是 A2DP 就直接进入第二步，不等待；当前已经是 A2DP 才连续观察 `3` 秒，始终保持 A2DP 即以“点击后自行恢复”结束，中途离开则进入第二步。该节点用于覆盖点击一键修复导致文本框失焦、语音输入自行结束的场景，避免已经恢复时仍执行修复动作并反过来重新触发格式请求。
...
- 点击按钮导致语音输入自行结束、第一步实时复核已无占用进程时，若即时模式已经是 A2DP 且随后连续保持 `3` 秒，整个流程不得执行任何输入输出切换、进程退出或声音链路重建；即时模式仍为 HFP/HSP 时必须直接进入第二步，不额外等待。
