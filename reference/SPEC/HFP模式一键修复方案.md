...
- 第一步实时复核没有麦克风进程可解除时，保持输入输出不变并立即读取一次目标模式。当前已经是 A2DP 才连续观察 `3` 秒；连续保持便结束，中途离开则进入第二步。即时模式不是 A2DP 时不等待，直接进入第二步。
...
没有麦克风进程可解除时增加一个即时检查节点：当前不是 A2DP 就直接进入第二步；当前已经是 A2DP 才观察是否连续保持 `3` 秒，保持成功立即结束，中途离开则进入第二步。该节点不等待目标从 HFP/HSP 进入 A2DP。
...
- 第一步无麦克风进程可解除后的即时检查是唯一例外：只在第一次读取已经是 A2DP 时开启连续 `3` 秒稳定窗；第一次读取不是 A2DP 时不进入等待窗。
...
- 第一步没有麦克风进程可解除、即时模式已经是 A2DP 且连续保持 `3` 秒后，不得执行第二步或任何后续修复动作。
