...
“声音输入进程正在运行”本身不等于“蓝牙麦克风被占用”。满足以下任一证据时，统一计入麦克风占用：一是存活进程明确关联到某个实体蓝牙麦克风端点；二是同一存活进程最后一次格式请求为 `0 -> 1`，且其后没有该进程对应的 `1 -> 0`。第二类证据一旦成立，主服务必须立即把该进程加入页面实时麦克风占用状态和一键修复现场，不得等用户点击修复后才查询；不要求系统已经报告实体端点、`StartIO` 或 `tsco`，但必须复核当前进程启动时间不晚于该请求，避免把已经复用的进程号误认成原请求者。设备列表为空、只关联 `AudioTap（系统声音抓取通道）`、内置输入、虚拟输入、回环输入或无法确认蓝牙实体端点的声音输入活动，只有在同时缺少未闭合 `0 -> 1` 时才不得进入麦克风占用处理。

麦克风占用处理是所有原因路由之前的前置动作：只要确认任意本机进程正在读取任意实体蓝牙麦克风，或存在存活进程的未闭合 `0 -> 1`，就先请求该进程退出，再观察目标设备是否稳定恢复 A2DP。目标已经稳定恢复时立即结束；未恢复时才根据退出后的最新输入、输出、链路和进程现场进入多端点会话、链路残留、其他格式请求或声音链路重建处理。不得在已确认麦克风占用仍存在时先要求用户选择保留输入或输出。
...
用户点击列表级按钮后，前端取该时刻最新设备列表中全部“模式为 `HFP_HSP` 且 A2DP 支持能力不是 `UNSUPPORTED`”的设备，按列表顺序逐台建立独立修复回合，禁止并行修改声音路由。每台设备开始处理时仍须立即保存该设备的点击现场并执行本文完整路由。某台设备进入等待授权或等待组合选择时，批次暂停；用户完成该设备的选择后自动继续其余仍需要修复的设备。前一个设备的处理已经使后续设备退出 HFP 时，后续设备不再执行修复动作。设备卡片只显示必须由用户继续操作的授权或组合选择，不显示普通进行态和已完成结果；批次进行态与最终结果统一显示在列表级“一键修复”胶囊内。
...
3. 实际修复首先检查是否存在“进程明确关联实体蓝牙麦克风端点”或“存活进程提交 `0 -> 1` 后没有对应 `1 -> 0`”任一完整证据；只要命中，不论当前输入输出是否多端点、是否存在 `StartIO`、该麦克风或目标最新链路是否为 `tsco`，都先结束对应进程并观察目标是否稳定恢复 A2DP。
4. 目标已经稳定恢复 A2DP 时结束本轮。目标未恢复时，重新采集输入、输出、每设备链路和声音输入进程现场，再进入后续原因匹配。
5. 前置处理后没有已确认蓝牙麦克风占用时，检查当前默认输入和默认输出是否来自两台不同的蓝牙设备，再检查目标设备最新链路是否为 `tsco`；两个条件同时成立时，才归为多端点会话类并进入组合选择。页面提前显示的多端点风险提示不等于原因确诊。
6. 未命中多端点会话类、但目标设备最新链路仍为 `tsco` 时，归为链路残留类并执行输入切换复位；不得因为存在设备列表为空的声音输入进程而改判占用。
7. 目标最新链路不是 `tsco` 或链路事实缺失时，才继续匹配格式请求类或证据不足类；格式请求证据不得覆盖当前已经明确成立的蓝牙麦克风占用、多端点或 `tsco` 残留。
8. 每个动作后立即观察目标输出；初步恢复即停止后续破坏性动作，再完成稳定性确认。
9. 若当前原因已经消失但仍未恢复，基于新状态先重新检查蓝牙麦克风占用，再判断多端点和其他原因；不得把上一次结论继续套用到变化后的现场。
...
flowchart TD
    ...
    B0 -->|是| A4[前端：当前设备计为成功并继续批次]
    B0 -->|否| C[后端：复核目标设备当前模式]
    C -->|已经退出 HFP| A1[前端：当前设备计为成功并继续批次]
    C -->|仍为 HFP| H[后端：根据最新现场和本轮处理记录匹配当前原因]

    H --> H0{本轮原因复查是否达到四次}
    H0 -->|否| H3{存在实体蓝牙麦克风读取，或未闭合 0→1 吗}
    H0 -->|是| U0{本轮是否已重连目标设备}

    H3 -->|是| I{本轮是否已处理过同一进程或进程族}
    H3 -->|否| H1{输入输出来自两台不同蓝牙设备吗}
    H1 -->|是| H2{目标设备最新链路为 tsco 吗}
    H1 -->|否| H4{目标设备最新链路为 tsco 吗}
    ...
    J -->|新快照不再确认同一实体蓝牙麦克风读取| J2[前端撤销过时授权并请求沿用原回合继续]
    J2 --> H
    J -->|用户授权| J0{后端重新确认同一进程仍有实体蓝牙麦克风读取或未闭合 0→1}
    J0 -->|否| H
    J0 -->|是| J1[后端阻止本次开机自动拉起]
    J1 --> D[后端：结束已确认的麦克风占用进程]
    I -->|已授权后仍再次占用| Z5[前端：当前设备计为错误]
    I -->|否，新进程| D
    D --> E{设备是否稳定恢复}
    E -->|是| Z[前端：当前设备计为成功]
    E -->|否| H
    ...
    Z6 --> A2
    A2 -->|有| A0
    A2 -->|没有| A3{前端：本批次是否出现错误}
    A3 -->|否| A31[前端：列表级胶囊显示成功]
    A3 -->|是| A32[前端：列表级胶囊显示错误]
    A31 --> A33[前端：10 秒后清除结果；仍有可修复设备则恢复一键修复，否则胶囊消失]
    A32 --> A33
```
...
- 每次进入“蓝牙麦克风占用 → 多端点 → `tsco` 残留 → 格式请求或证据不足”的完整匹配都计为一次原因复查；一次点击最多执行四次原因复查。授权后的继续操作属于同一修复回合，不重置次数。
...
- 麦克风占用的等待授权只在所列进程仍明确关联同一实体蓝牙麦克风端点，或同一存活进程仍存在未闭合 `0 -> 1` 时有效，不要求设备最新链路为 `tsco`。主服务持续维护两类证据并写入普通设备快照；实体端点证据消失或同一进程收到后续 `1 -> 0`、退出、进程号被复用时，前端根据较新的占用快照撤销过时授权。用户继续操作时后端仍须实时复核。两类证据都消失后沿用原修复回合继续原因匹配，不能仅凭页面保存的等待状态阻止进程。
...
- 第一优先级是麦克风占用前置门槛：进程明确关联实体蓝牙麦克风端点，或同一存活进程提交 `0 -> 1` 后没有后续对应 `1 -> 0`，任一条件完整命中即可；不要求 `StartIO` 或设备最新链路为 `tsco`。命中后先结束对应进程并观察目标是否稳定恢复 A2DP。仅有运行中的声音输入流、进程身份、内置输入或空设备列表仍不足，但它们不得否定已经成立的未闭合 `0 -> 1`。
- 第二优先级是多端点门槛：前置占用处理后目标仍未恢复、当前既没有已确认实体蓝牙麦克风读取也没有存活进程的未闭合 `0 -> 1`、默认输入和默认输出来自两台不同蓝牙设备，并且目标设备最新链路为 `tsco`，才归为多端点会话类。前端风险提示只基于设备组合提前告警，不参与服务端确诊。
- 第三优先级是链路残留门槛：既没有已确认实体蓝牙麦克风读取，也没有存活进程的未闭合 `0 -> 1`，但目标设备最新链路为 `tsco`，直接归为链路残留类。此前是否抓到占用进程可以作为案例证据，但不是当前路由的必要条件。
...
- 链路残留类以“当前没有已确认蓝牙麦克风占用 + 目标设备最新链路为 `tsco`”为完整进入条件。进程设备列表为空、内置输入、`AudioTap`、虚拟输入或未归属声音输入活动均视为“没有已确认蓝牙麦克风占用”，不得阻挡残留类路由。
- 用户主动修复时，当前双蓝牙输入输出组合本身只构成风险；先处理全部已确认蓝牙麦克风占用，目标未恢复且当前已经没有确认占用时，再确认目标设备最新链路为 `tsco`，才进入多端点设备组合选择。应用名只是可选展示，不是允许切换设备的证据门槛。
...
- 未闭合 `0 -> 1` 前后 `2` 秒内，若只有一台具有蓝牙输入端点的设备出现地址已归属的 `tsco` 事实，页面必须把该进程放入这台设备卡片的“麦克风占用”，并显示进程名称、进程号、请求时间和“最后一次格式请求未释放”。没有唯一设备匹配时才在设备列表上方显示全局麦克风占用，且不得伪造具体麦克风名称，也不得继续显示为“其他声音输入活动”或“未确认麦克风占用”。同一进程收到后续 `1 -> 0`、退出或进程号被复用后，页面必须立即清除该占用。
- 设备卡片名称下方的胶囊显示 A2DP、HFP/HSP 或“模式无法确认”，具体判定统一遵守 [`如何判定蓝牙音频设备的音频模式.md`](如何判定蓝牙音频设备的音频模式.md)。只要进程明确关联该实体蓝牙麦克风端点，就附加显示“麦克风使用中”，不要求该设备最新链路为 `tsco`；卡片右侧只保留展开图标。
- 用户点击时若目标仍为 HFP，服务端先结束所有已确认正在读取实体蓝牙麦克风的本机进程并观察目标是否稳定恢复 A2DP。目标未恢复时，再检查当前输入输出是否来自两台不同的经典蓝牙设备，并确认目标设备最新链路是否为 `tsco`；两项同时成立后，提示“当前双蓝牙输入输出组合仍使设备处于 HFP”，并请用户选择保留输入或保留输出。用户点选前不得改动路由；点选后直接切换对应设备并验证。
- 模式展示保留在各设备卡片内，修复动作只放在设备列表状态栏内，两者必须是独立控件。“一键修复”使用原生按钮；有 HFP 设备且当前没有修复批次、待处理选择或仍在展示的最终结果时显示，执行期间禁用并沿用现有批次进度文案，不得把修复动作藏在模式文字的悬停态中，也不得在设备卡片内重复放置修复按钮。
- 列表级批次必须逐台执行，不得同时对多台设备结束进程、切换输入输出或重连蓝牙。某台设备等待授权或等待组合选择时，统一按钮保持禁用；用户完成该选择后沿用原批次自动处理剩余目标。设备卡片只保留等待授权和等待组合选择等必须交互的区域，不显示普通进行态、成功、错误或无需修复结果模块。
...
- 多端点设备组合和目标 `tsco` 同时成立时，若仍有已确认实体蓝牙麦克风占用，必须先处理占用，不得同时建立路由选择。占用解除后目标仍未恢复、当前已经没有确认占用且多端点条件仍成立时，目标输出卡片自动展开并展示路由选择。
- 原因进程未退出，或退出后再次启动并触发同一问题时，页面必须先列出涉及的全部进程名称，再单独请求“仅限本次开机”的继续处理与阻止重新启动授权；确认提示中也必须重复列出进程名称。未授权时停止，不得擅自继续结束进程。
- 蓝牙麦克风占用进入等待授权后，页面收到晚于等待结果的新快照时，必须核对所列进程是否仍明确关联同一实体蓝牙麦克风端点，不要求该设备最新链路为 `tsco`。关联仍成立才保留授权；关联不成立就立即撤销授权，并自动请求服务端沿用原点击现场、原因复查次数和动作记录继续匹配，不得生成新的修复回合。
- 修复过程中首次观察到高于 `16 kHz` 时，页面立即把进行中提示改为“正在确认稳定”，不得等整个请求结束后才显示。
- 点击后必须在发送请求前同步把列表级胶囊改为进行中状态并禁用重复提交；刷新标签页只恢复仍待用户处理的授权或组合选择及其批次队列，不恢复进行中临时状态或已经结束的成功、错误结果。
...
- 一键修复和单独解除占用都必须显示当前阶段，不得只显示无变化的旋转状态。一键修复批次结束后，列表级胶囊只显示“成功”或“错误”，保留 `10` 秒后清除；清除时仍有可修复 HFP 设备则恢复“一键修复”，否则胶囊消失。单独解除麦克风占用成功后仍在占用区显示确认结果，并在 `10` 秒后自动消失；复查期间不继续锁住按钮。
...
- 标签页刷新后不恢复已完成历史结果；只有“等待组合选择”或“等待授权”的交互区域连同原批次状态一起恢复并自动展开。
- 等待组合选择期间收到更新的设备快照时，若目标已经退出 HFP/HSP、已确认不支持 A2DP 或已经不在当前设备列表中，前端必须立即清除过期选择和对应标签页存储，并沿用原批次继续处理剩余设备；不得继续显示旧的 `tsco` 原因或保留输入输出按钮。
- 授权或组合选择属于同一项一键修复功能的后续步骤；继续执行时仍要复核实时现场，旧结论失效时不得强行执行旧动作。
- 用户点击蓝牙麦克风占用授权时，服务端必须实时重读进程和实体输入端点，确认待授权的同一进程或进程族仍关联同一实体蓝牙麦克风。无法归属蓝牙实体设备或已经停止读取时，不启动本次开机阻止任务，直接沿用原回合重新匹配当前原因。
...
- 用户已经点击某个多端点替代组合，但执行前目标自行恢复时，本设备在批次中按成功结束，但不得生成暗示设备已经切换的卡片结果文案。
- 批次最终结果只允许显示在列表级“一键修复”胶囊内：所有设备均未出错时显示“成功”，任一设备失败或请求异常时显示“错误”；不显示时间、原因、动作、采样率、工作流、诊断证据或执行步骤。等待用户选择的操作按钮仍直接显示在对应设备卡片中。
- 若处理对象是输入法等常驻进程，等待授权区域可以说明“进程重新启动不等于语音快捷键已经恢复”；批次最终胶囊仍只显示“成功”或“错误”，不得追加应用功能已经完全恢复的声明。
...
1. 某进程仍在运行并具有当前声音输入活动。
2. 该进程明确关联到一个实体蓝牙麦克风端点，而不是设备列表为空、内置输入、虚拟输入、回环输入或系统声音抓取通道。

两个条件完整成立，才能确认“某进程占用某蓝牙麦克风”，进而进入所有后续原因路由之前的占用处理；不要求该设备最新链路为 `tsco`。进程存在、进程有声音输入流或能够读取进程路径，都不能代替实体蓝牙麦克风端点关联证据。
...
- 同一应用或同一进程族短时间内自动重启，并再次读取同一实体蓝牙麦克风：告知用户该进程反复重启，询问是否授权阻止它在本次开机期间继续自动拉起；不得无限结束进程。
- 进程未能退出，且仍读取同一实体蓝牙麦克风：告知用户该进程未退出，不得写成“退出后再次启动”或“再次读取”；仍须在用户授权前实时确认蓝牙麦克风占用没有消失。
- 占用已经消失但目标仍未恢复 A2DP：回到“匹配当前原因”，先检查多端点门槛，未命中时再进入链路残留或其他后续处理。

授权只限本次开机期间；不得修改登录项、开机自启、永久禁用、删除应用或改变下次重启后的长期配置。

## 链路残留类

进入条件：当前没有任何满足“进程明确关联实体蓝牙麦克风端点”的已确认占用；当前输入输出没有命中多端点会话类；目标设备最新独立链路事实为 `tsco`。三项成立即归为链路残留类并路由处理。
...
进入条件：蓝牙麦克风占用前置处理后目标仍未恢复 A2DP，且当前没有已确认蓝牙麦克风占用；再确认当前默认输入和默认输出来自两台不同的蓝牙设备，并确认目标设备最新独立链路事实为 `tsco`。这些条件必须同时成立，才归为多端点会话类。该类低于蓝牙麦克风占用处理，高于残留类和格式请求类。

产品边界：这是具体应用与跨蓝牙输入输出组合不兼容，不是本工具可以修改的应用内部故障。本工具的修复责任只是识别拒绝证据、实时呈现冲突，并在用户授权后改成可用路由组合。

前端在发现输入输出来自两台不同蓝牙设备时先显示风险提示；风险提示已经存在不等于完成确诊。用户实际点击一键修复后，服务端先处理已确认蓝牙麦克风占用；目标未恢复且当前已无确认占用时，只有再次确认该组合仍存在且目标最新链路为 `tsco`，才报告“当前双蓝牙输入输出组合仍使设备处于 HFP”并提供组合选择。
...
进入条件：没有完整命中蓝牙麦克风占用、“多端点 + `tsco`”、`tsco` 链路残留或格式请求中的任何已确证原因，但目标仍被模式判定功能判为 HFP/HSP。
...
- 实际修复是否先检查进程是否明确关联实体蓝牙麦克风端点，并在不要求 `tsco` 的情况下先结束对应进程、观察目标是否稳定恢复 A2DP。
- 目标未恢复且当前已无确认蓝牙麦克风占用后，是否只有“两台不同蓝牙设备 + 目标 `tsco`”同时成立才进入多端点会话类。
- 没有已确认蓝牙麦克风占用、但目标最新链路为 `tsco` 时，是否归为链路残留类并执行输入切换复位。
...
- 普通进行态和已完成结果是否完全不生成设备卡片模块；列表级胶囊是否在批次结束后只显示“成功”或“错误”，并在 `10` 秒后清除，随后按当前可修复设备数决定恢复“一键修复”还是消失。
- 单独解除麦克风占用成功提示是否在 `10` 秒后消失，期间若重新占用是否改为最新警告而不被旧定时器删除。
...
- 蓝牙麦克风占用等待授权后占用消失时，前端是否撤销过时授权并沿用原回合继续；用户点击授权时后端是否再次确认同一进程仍在读取同一实体蓝牙麦克风，未确认时是否避免启动阻止任务。
- 声音输入进程无法归属实体设备时，是否只显示为页面级“未归属声音输入活动”；已确认的 `AudioTap` 等系统声音采集是否与麦克风占用分开展示，且两者均没有解除占用或授权按钮。
- 多端点会话是否在蓝牙麦克风占用前置处理后仍未恢复，并同时满足当前无确认占用、设备组合和目标 `tsco`，随后只提供路由组合修复。
- 双蓝牙路由反复断连或模式切换时，前端是否仍跟随每次瞬时事件立即刷新，不延迟、合并或丢弃麦克风占用等中间状态。
- 同一存活进程最后一次格式请求为 `0 -> 1` 且没有后续 `1 -> 0` 时，是否先按请求前后 `2` 秒内唯一的设备级 `tsco` 事实归入对应设备卡片，仅在无法唯一匹配时显示页面级占用；该请求闭合、进程退出或进程号复用后是否立即清除；是否没有伪造具体设备归属。
- 是否只在两个蓝牙端点和目标 `tsco` 同时成立后才确诊多端点会话；能够点名应用时是否只把应用身份作为补充信息，并在用户点选前保持只读。
- 蓝牙麦克风占用与多端点条件同时存在时，是否先处理占用且不建立路由选择；占用解除后仍满足多端点条件时，是否立即停止显示“正在确认”、自动展开并展示路由选择。
