...
```mermaid
flowchart TD
    A[前端：用户点击一键修复] --> A0[前端：立即显示处理中并禁用重复点击，只提交目标设备身份]
    A0 --> B[后端：立即保存点击时输入、输出、占用和模式快照]
    B --> C[后端：复核目标设备当前模式]
...
- 麦克风占用类的等待授权只在所列进程仍被最新占用检查确认为读取者时有效。更新的占用快照确认所列进程均已停止读取时，前端必须撤销过时授权并沿用原修复回合继续原因匹配；用户点击授权时后端仍须独立补做一次实时占用确认，不能仅凭页面保存的等待状态阻止进程。
...
- 用户点击麦克风占用类授权时，服务端必须实时重读全部本机麦克风读取者，并以当前进程路径确认待授权的同一进程或进程族仍在读取。若已经停止读取，不启动本次开机阻止任务，直接沿用原回合重新匹配当前原因。
