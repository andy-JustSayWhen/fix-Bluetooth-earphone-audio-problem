...
1. 将默认输入临时切到任意可用的非蓝牙输入，最多等待 `500` 毫秒观察目标独立链路是否转为 `tacl`；若出现，从出现时刻起固定保持中转输入 `1` 秒，若未出现则记录失败并立即恢复点击前默认输入。只验收恢复后的原输入输出组合，不验收中转状态。可用中转包括内置麦克风、USB 麦克风、2.4G 接收器声音输入或其他非蓝牙声音输入。没有可用非蓝牙输入就跳过；不得切到另一台蓝牙麦克风作为中转。等待原因和当前数值依据统一见[中转输入等待参数依据](#中转输入等待参数依据)。
