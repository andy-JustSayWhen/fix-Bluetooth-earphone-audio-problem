...
- 点击一键修复可能使依赖文本框焦点的语音功能立即结束。第一步实时复核已经没有可结束的实体蓝牙麦克风占用进程时，必须先保持当前输入输出不变，执行一次稳定恢复观察；观察成功便以“点击后自行恢复”结束，不得继续切换设备、结束格式请求进程或重建声音链路。观察失败才进入第二步。该保护只位于第一步与第二步之间，不在其他跳过节点重复增加等待。
...
实时复核没有可结束的占用进程时，不得立刻操作声音路由。先保持现场不变观察一次：最多等待 `3` 秒让目标首次进入 A2DP，首次进入后连续保持 `3` 秒即以“点击后自行恢复”结束；否则才进入第二步。该节点用于覆盖点击一键修复导致文本框失焦、语音输入自行结束的场景，避免修复动作反过来重新触发格式请求。
...
- 点击按钮导致语音输入自行结束、第一步实时复核已无占用进程时，目标若随后稳定恢复 A2DP，整个流程不得执行任何输入输出切换、进程退出或声音链路重建。
