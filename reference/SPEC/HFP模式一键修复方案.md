...
“声音输入进程正在运行”本身不等于“蓝牙麦克风被占用”。满足以下任一证据时，统一计入麦克风占用：一是存活进程明确关联到某个实体蓝牙麦克风端点；二是同一存活进程提交 `0 -> 1` 格式请求后，在其后没有该进程对应的 `1 -> 0`。第二类证据不要求系统已经报告实体端点、`StartIO` 或 `tsco`，但必须复核当前进程启动时间不晚于该请求，避免把已经复用的进程号误认成原请求者。设备列表为空、只关联 `AudioTap（系统声音抓取通道）`、内置输入、虚拟输入、回环输入或无法确认蓝牙实体端点的声音输入活动，只有在同时缺少未闭合 `0 -> 1` 时才不得进入麦克风占用处理。
...
flowchart TD
    ...
    J -->|新快照不再确认同一实体蓝牙麦克风读取或未闭合 0→1| J2[前端撤销过时授权并请求沿用原回合继续]
    J2 --> H
...
- 麦克风占用的等待授权只在所列进程仍明确关联同一实体蓝牙麦克风端点，或同一存活进程仍存在未闭合 `0 -> 1` 时有效，不要求设备最新链路为 `tsco`。两类证据都消失时，前端必须撤销过时授权并沿用原修复回合继续原因匹配；用户点击授权时后端仍须独立补做一次实时复核，不能仅凭页面保存的等待状态阻止进程。设备列表为空或只发现内置输入、系统声音采集活动时，若同一进程仍有未闭合 `0 -> 1`，授权仍可建立。
