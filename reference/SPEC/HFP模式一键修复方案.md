...
- 麦克风占用的等待授权只在所列进程仍明确关联同一实体蓝牙麦克风端点，或同一存活进程仍存在未闭合 `0 -> 1` 时有效，不要求设备最新链路为 `tsco`。实体端点证据消失时，前端可根据较新的占用快照撤销过时授权；未闭合格式请求不在普通设备快照中，前端不得仅因实体端点列表为空自动撤销，用户继续操作时必须由后端重新查询系统声音日志。两类证据都消失后沿用原修复回合继续原因匹配，不能仅凭页面保存的等待状态阻止进程。
