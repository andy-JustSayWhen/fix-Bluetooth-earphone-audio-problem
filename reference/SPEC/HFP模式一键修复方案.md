...
- 页面必须在设备列表上方统一显示未闭合 `0 -> 1` 形成的麦克风占用，列出进程名称、进程号、请求时间和“最后一次格式请求未释放”。该证据没有实体端点归属时不得伪造具体麦克风名称，也不得继续显示为“其他声音输入活动”或“未确认麦克风占用”。同一进程收到后续 `1 -> 0`、退出或进程号被复用后，页面必须立即清除该占用。
...
- 同一存活进程最后一次格式请求为 `0 -> 1` 且没有后续 `1 -> 0` 时，页面是否立即在设备列表上方显示麦克风占用；该请求闭合、进程退出或进程号复用后是否立即清除；是否没有伪造具体设备归属。
