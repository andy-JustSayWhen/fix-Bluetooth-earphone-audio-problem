...
- 多台目标按页面顺序串行处理。
- 每台开始时保存：点击时间、目标设备、默认输入输出、实际输出采样率、模式、每设备最新 `tacl/tsco`、进程与实体输入端点关联、未闭合格式请求、声音会话端点与进程身份。
- 不早于点击前 `2` 秒的快照可以复用；过期时只补查一次。
- 每个动作前复核目标仍为 HFP/HSP；已恢复则结束本设备回合。
...
结束全部已确认占用进程，最多等待 `2` 秒确认退出，再观察恢复。进程未退出或重新形成同一占用时，显示进程名称并等待“仅限本次开机”的阻止自动拉起授权；授权有效 `30` 分钟，只允许再处理一次。
...
系统日志只查本次点击后的时间窗，查询最长 `7.5` 秒。证据完整时结束请求进程，最多等待 `2` 秒确认退出，再观察恢复。进程未退出或再次提交时，可以等待一次“仅限本次开机”授权；授权后仍失败则进入第五步。
...
9. 等待目标端点出现，恢复仍可用的点击前输入输出，执行最终观察。

蓝牙开关和每个服务重启分别最多等待 `5` 秒，每 `100` 毫秒检查状态或新旧进程号。任一步失败仍继续，并保证最终尝试打开蓝牙和连接目标。目标连接调用最长 `18` 秒，调用后最多等待端点 `3` 秒。不得再执行独立断开重连。

## 通用等待与验收

- 默认输入或输出切换：最多 `2` 秒，每 `100` 毫秒检查一次。
- 中转链路：最多 `500` 毫秒，每 `50` 毫秒检查一次；出现 `tacl` 后保持中转输入 `1` 秒。
- 稳定恢复：最多观察 `10` 次，每次间隔 `500` 毫秒；连续 `3` 次满足才成功。
- 目标是当前默认输出：实际输出高于 `16 kHz` 且模式为 A2DP。
- 目标不是当前默认输出：模式不再是 HFP/HSP。
- 只有点击前仍可用的输入输出已恢复，并通过稳定观察，才报告成功。

## 交互与安全

- 页面只提交目标设备身份；服务端决定证据和动作。
- 页面展示的已确认格式请求占用必须与服务端解除资格一致。服务端复核到请求仍未闭合且进程身份仍有效时，直接请求该进程正常退出；不得再用“当前是否正在读取实体麦克风”或“能否归属具体麦克风”过滤掉它。
- 只要进程带有未闭合格式请求证据，页面在设备卡片、解除确认、重新占用提示和一键修复进度中显示该进程时，名称后统一追加“（格式请求）”；用于进程身份复核的原始名称和进程号保持不变。
- 批次进行态和最终“成功/错误”只显示在列表级胶囊；结果保留 `10` 秒。
- 等待授权时暂停批次并保存当前标签页队列；刷新后续接原回合。授权过期后必须重新点击。
- 普通原因步骤只能结束证据明确归属的应用进程。`audioaccessoryd`、`audiomxd`、`bluetoothd`、`bluetoothuserd`、`coreaudiod`和其他系统核心进程只能在第六步按服务身份重启。
