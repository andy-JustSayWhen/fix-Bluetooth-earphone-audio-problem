...
flowchart TD
    ...
    O0 -->|否| O[后端：切到任意非蓝牙输入]
    O0 -->|是| U0
    O --> O1[后端：保持中转输入 500 毫秒]
    O1 -->|不验收中转状态| P[后端：切回点击前输入]
    P --> Q{原输入输出组合稳定恢复吗}
    Q -->|是| Z
    Q -->|否| H

    R --> R0[后端：保持中转输入 500 毫秒]
    R0 -->|不验收中转状态| S[后端：切回点击前输入]
    S --> T{原输入输出组合稳定恢复吗}
    T -->|是| Z
    T -->|否| U0
...
- 链路残留和声音链路重建的中转输入固定保持 `500` 毫秒；这段时间属于输入复位动作，不在中转状态验收恢复结果。
- 不盲试所有方案。
- 不在每步前重新查完整系统日志。
...
2. 切换命令完成后保持中转输入 `500` 毫秒，再恢复点击前默认输入；中转期间不验收采样率，也不把临时出现的 A2DP 当作恢复结果。
...
1. 将默认输入临时切到任意可用的非蓝牙输入，保持中转输入 `500` 毫秒后恢复点击前默认输入；只验收恢复后的原输入输出组合，不验收中转状态。可用中转包括内置麦克风、USB 麦克风、2.4G 接收器声音输入或其他非蓝牙声音输入。没有可用非蓝牙输入就跳过；不得切到另一台蓝牙麦克风作为中转。
...
- 链路残留类和声音链路重建的非蓝牙中转输入是否固定保持 `500` 毫秒后才恢复点击前输入，且只验收恢复后的原组合。
