...
一键修复使用设备列表级统一入口，不在单个设备卡片内重复放置按钮。状态栏在“更新于……”字段后同时显示处于 HFP 的设备总数和其中需要修复的设备数；只有至少一台设备需要修复时才显示“一键修复”按钮。入口和服务端都使用模式判定功能给出的权威模式与 A2DP 支持能力：模式为 `HFP_HSP` 且支持能力不是 `UNSUPPORTED` 的设备进入修复；支持能力为 `UNSUPPORTED` 的设备明确显示“不支持A2DP（该设备无需修复）”，不得进入修复或产生“修复失败”。仅有 `16 kHz` 输入、但模式没有被判为 `HFP_HSP` 的设备不得计入数量；未播放的同名输出端点也不能单独构成 HFP/HSP 证据。
...
- 状态栏是否在更新时间后准确显示 HFP 设备总数和需要修复的设备数，并且只在需要修复的数量大于零时显示唯一的列表级“一键修复”按钮；设备卡片内是否不再重复显示按钮。
