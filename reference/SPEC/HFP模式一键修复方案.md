...
蓝牙开关和每个服务重启分别最多等待 `5` 秒，每 `100` 毫秒检查状态或新旧进程号。关闭蓝牙命令返回不等于关闭成功；只有独立读取到控制器电源状态已经变为关闭，才允许继续重启声音链路服务。`5` 秒内未确认关闭时，不得在蓝牙仍开启的现场冒充完整重建；应确保蓝牙保持开启，记录失败并结束本次修复。蓝牙已确认关闭后，后续任一步失败仍继续，并保证最终尝试打开蓝牙和连接目标。目标连接调用最长 `18` 秒，调用后最多等待端点 `3` 秒。不得再执行独立断开重连。
...
Windows 的恢复行为以 [Windows适配.md](Windows适配.md) 为原文；本文涉及苹果系统接口的步骤适用于 macOS。
