...
- 多台目标按页面顺序串行处理。
- 每台开始时保存：点击时间、目标设备、默认输入输出、实际输出采样率、模式、每设备最新 `tacl/tsco`、进程与实体输入端点关联、未闭合格式请求及其请求后 `2` 秒内的 `tsco/StartIO` 证据、声音会话端点与进程身份。
...
第四步先使用点击现场保存的未闭合格式请求和对应证据，再合并点击后新产生的格式请求、反向恢复、`tsco` 与 `StartIO`。不得因为请求发生在点击前而丢弃仍未闭合的请求，也不得重新扩大查询到与点击现场无关的更早历史。需要补查系统日志时，查询最长 `7.5` 秒。
