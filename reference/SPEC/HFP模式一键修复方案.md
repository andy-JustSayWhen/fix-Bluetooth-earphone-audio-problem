...
1. 复核目标仍被最新模式结论判为 HFP/HSP；若现场已退出 HFP/HSP，直接结束，不查询已经结束的会话，也不继续任何修复动作。
2. 不论最终属于哪类原因，先检查并尝试一次解除当前所有已确证的本机麦克风占用。若目标输出因此稳定恢复，立即结束。
3. 本轮已经确认并结束蓝牙输入占用、没有新的麦克风读取者，但设备仍为 HFP 时，立即转为链路残留类：先临时切换到非蓝牙输入并恢复原输入；该动作仍未恢复，才继续匹配多端点会话、格式请求或执行重连等其他方法。点击时本来就没有实际占用时，才直接匹配当前原因。
...
flowchart TD
    ...
    C -->|占用已结束但仍未恢复| L[链路残留类：切换非蓝牙输入再切回]
    C -->|点击时没有实际占用| K[匹配当前原因]
    K -->|多端点会话类| E[修正输入输出组合]
    K -->|格式请求类| F[退出格式请求进程]
    K -->|历史证据命中链路残留类| L
    K -->|证据不足| G[声音链路重建兜底]
...
- 链路残留类以同一修复回合的状态变化为首要证据：先确认进程实际读取蓝牙输入端点，再确认占用结束且没有新的读取者，最后确认目标设备或其他蓝牙设备仍保持 HFP。该证据链一旦成立，必须先执行链路残留类的输入切换处理，不得先跳到多端点、格式请求或重连。只有旧日志中存在输入启动和 `tsco`、但无法对应当前设备或确认占用已经结束时，只能标为高度疑似。
...
- 占用已经消失但仍处于 HFP：本轮已经确认并结束蓝牙输入占用时，立即转为链路残留类并先执行一次输入切换；该动作无效后才重新按原因实例文档匹配多端点会话类、格式请求类或继续重连。
...
1. 将默认输入临时切换到非蓝牙输入，解除原蓝牙输入端点与当前声音路由的绑定。
2. 中转输入保持期间，每 `500 ms` 检查受影响的当前默认输出；连续三次高于 `16 kHz` 后，才恢复点击前默认输入。不得在第一次高采样率出现后立即切回。
3. 恢复点击前输入后，继续按 A2DP 模式和高采样率连续确认。
4. 若原组合仍未恢复或再次进入 HFP，才重新匹配当前多端点会话、格式请求等原因；没有其他可处理原因时，再断开并重新连接目标蓝牙设备。
5. 最终必须恢复点击前输入输出并通过稳定确认，才能报告完全恢复；临时停留在中转输入只能报告绕过成功。
...
1. 将默认输入临时切到任意可用的非蓝牙输入；目标是当前默认输出时，先连续三次确认实际输出高于 `16 kHz`，再恢复点击前默认输入。可用中转包括内置麦克风、USB 麦克风、2.4G 接收器声音输入或其他非蓝牙声音输入。没有可用非蓝牙输入就跳过；不得切到另一台蓝牙麦克风作为中转。
