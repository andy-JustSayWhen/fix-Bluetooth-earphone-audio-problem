...
- 稳定恢复只判断目标模式：每个实际修复步骤结束后，每 `500` 毫秒读取一次主服务持续更新的当前设备评估，最多等待 `3` 秒让目标首次进入 A2DP。首次进入 A2DP 时开始该步骤独立的连续 `3` 秒稳定窗；稳定窗内任一次离开 A2DP，均判为本步骤未成功并直接进入下一步骤。目标连续保持 A2DP 满 `3` 秒即成功结束一键修复。等待期内始终没有进入 A2DP，也直接进入下一步骤。
