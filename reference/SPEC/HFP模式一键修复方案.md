...
- 稳定恢复只判断目标模式：每个实际修复步骤结束后立即开始该步骤独立的 `3` 秒观察窗，每 `500` 毫秒读取一次主服务持续更新的当前设备评估。从观察窗第一次读取到最后一次读取，目标必须始终为 A2DP；第一次不是 A2DP、期间任一次离开 A2DP 或结束时不是 A2DP，均判为该步骤未成功恢复并直接进入下一步骤。下一步骤执行完成后重新开始新的独立 `3` 秒观察窗。
