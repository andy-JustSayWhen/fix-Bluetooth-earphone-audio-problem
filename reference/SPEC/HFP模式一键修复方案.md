...
- 当前默认输入和默认输出分别来自两台不同的经典蓝牙设备时，页面必须在用户发起语音操作前显示风险提示；提示只陈述当前组合和可能失败，不得在没有系统日志证据时提前冒充“多端点会话类”确诊。
- 当同一双蓝牙输入输出组合在短时间内反复断连，或目标输出在 A2DP 与 HFP 间反复切换时，前端必须进入稳定展示状态：保留最近一份稳定设备卡片，不跟随每一次瞬时事件在模式、断连和重连之间反复重绘。只有路由稳定、用户主动换路由，或当前组合被修复后才解锁展示。
- 前端观察到上述抖动后，可自动发起一次只读的最近多端点证据复核。未确诊时只报告“尚不能确认具体应用”，不结束进程、不切换设备、不进入声音链路重建。
...
产品边界：这是具体应用与跨蓝牙输入输出组合不兼容，不是本工具可以修改的应用内部故障。本工具的修复责任只是识别拒绝证据、稳定呈现冲突，并在用户授权后改成可用路由组合。
...
- 多端点会话是否只提供路由组合修复。
- 双蓝牙路由反复断连或模式切换时，前端是否锁住最近稳定展示，而不是跟随瞬时事件反复重绘。
