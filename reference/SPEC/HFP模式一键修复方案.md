...
3. 没有实际占用，或解除一次后仍未恢复，先匹配当前多端点会话和格式请求；若本轮已经确认并结束蓝牙输入占用、没有新的麦克风读取者或其他已确证原因，但设备仍为 HFP，则转为链路残留类；仍无完整证据才进入兜底。
...
- 链路残留类以同一修复回合的状态变化为首要证据：先确认进程实际读取蓝牙输入端点，再确认占用结束且没有新的读取者，最后确认目标设备或其他蓝牙设备仍保持 HFP。只有旧日志中存在输入启动和 `tsco`、但无法对应当前设备或确认占用已经结束时，只能标为高度疑似。
...
- 占用已经消失但仍处于 HFP：重新按原因实例文档匹配；存在更新的多端点会话类或格式请求类证据时处理新原因，否则本轮已经确认并结束蓝牙输入占用时转为链路残留类。
...
3. 恢复点击前输入后，继续按 A2DP 模式和高采样率连续确认；若原组合复发，再断开并重新连接目标蓝牙设备。
4. 最终必须恢复点击前输入输出并通过稳定确认，才能报告完全恢复；临时停留在中转输入只能报告绕过成功。
