...
- 稳定恢复只判断目标模式：动作完成后最多等待 `5` 秒进入 A2DP；进入 A2DP 后必须连续保持至少 `3` 秒才成功。保持期间一旦离开 A2DP，连续计时立即清零；首次等待期已经结束时直接判为未稳定，否则继续等待目标重新进入 A2DP。一次稳定观察最长 `8` 秒。
- 稳定观察每 `500` 毫秒读取一次主服务持续更新的当前设备评估，不按默认输出身份、采样率或检查次数另设分支，也不得在每次观察中重新执行完整设备扫描。
