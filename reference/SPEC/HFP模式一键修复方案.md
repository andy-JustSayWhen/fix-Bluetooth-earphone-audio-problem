...
2. 未恢复时，把当前默认输入 A 切换到其他非蓝牙输入 C，再切回 A，然后观察。
3. 未恢复时，把当前默认输入 A 和默认输出 B 分别切换到其他非蓝牙输入 C、输出 D，再切回 A、B，然后观察。
4. 未恢复时，检查仍有效的格式请求；若存在，结束请求进程并观察。
5. 未恢复时，检查默认输入和默认输出是否来自两台不同蓝牙设备；若是，再检查是否存在触发 `more than one BT device connected` 的应用进程。能确认进程时先结束该进程；随后执行一次中转输入切换并切回，再观察。
6. 未恢复时，严格执行一次完整声音链路重建：关闭蓝牙 → 重启 `bluetoothd（管理全机蓝牙连接的系统进程）` → 重启 `bluetoothuserd（管理当前用户蓝牙状态的系统进程）` → 重启 `coreaudiod（系统核心声音后台进程，同时重载蓝牙声音插件）` → 重启 `audioaccessoryd（管理声音配件状态和路由协作的系统进程）` → 重启 `audiomxd（管理应用声音会话的系统进程）` → 打开蓝牙 → 连接目标设备 → 恢复点击前路由并完成最终观察。
7. 仍未恢复时结束本轮并报告错误，不回到前面的步骤，不重复同一步骤。
...
flowchart TD
    ...
    C -->|是| D{有实时蓝牙麦克风占用吗}
    D -->|是| D1[结束占用进程并观察]
    D -->|否| U[默认输入 A 切到非蓝牙 C 后切回 A]
    D1 --> R1{稳定恢复了吗}
    R1 -->|是| Z
    R1 -->|否| U
    U --> R2{稳定恢复了吗}
    R2 -->|是| Z
    R2 -->|否| V[输入 A 和输出 B 切到非蓝牙 C、D]
    V --> V1[先切回输出 B，再切回输入 A]
    V1 --> R3{稳定恢复了吗}
    R3 -->|是| Z
    R3 -->|否| E{有仍有效的格式请求吗}
    E -->|是| E1[结束格式请求进程并观察]
...
“一键修复”在任何步骤中需要把输入或输出切换到“其他非蓝牙设备”时，必须只使用本节规则。其他章节只能引用本节，不得定义另一套候选范围或优先级。

候选必须具有所需方向的有效声道、当前可用、不是准备替换的原设备，并且传输类型不是蓝牙或低功耗蓝牙。固定优先级为：

1. 内置设备。
2. 明确标记的其他有线或接收器设备，包括 USB、2.4G 接收器、显示器声音、HDMI、雷雳、火线、PCI、线路和数字声音设备。
3. 其他未标记为蓝牙的设备。

强制规则：

- 永远不得把其他蓝牙设备作为“其他非蓝牙设备”的降级候选。三个级别都没有可用设备时，记录“没有可用的其他非蓝牙设备”，跳过当前切换步骤并继续下一步骤。
- 只有当前级别没有候选，或当前级别全部切换失败时，才能进入下一级。同一级按系统设备顺序尝试。
- 输入候选和输出候选分别计算。输入 C 与输出 D 可以来自同一台同时具有输入、输出能力的非蓝牙设备，也可以来自两台设备；两者都必须各自满足本节优先级。
- 候选在执行前失效时，必须根据最新设备列表重新计算，不得沿用旧候选或降级为蓝牙设备。
- 每次等待候选成为默认输入或默认输出最多 `2` 秒，每 `100` 毫秒检查一次。任何部分切换失败时，必须恢复已经改变的点击前路由，再进入下一步骤。
...
设备列表为空、内置输入、虚拟输入、回环输入、`AudioTap（系统声音抓取通道）`或无法归属实体设备的声音输入活动，都不算本步骤的实时蓝牙麦克风占用。格式请求也不再并入实时占用，统一留到第四步单独判断。
...
本步骤开始时保存当前默认输入为 A，并按[强制统一约束：选择其他非蓝牙设备](#强制统一约束选择其他非蓝牙设备)选择其他非蓝牙输入 C。

1. 把默认输入从 A 切换到 C，并确认 C 已成为默认输入。
2. 最多等待 `500` 毫秒观察目标链路转为 `tacl`，每 `50` 毫秒检查一次，并在边界再检查一次。
3. 若观察到 `tacl`，从出现时刻起继续保持 C `1` 秒；若没有观察到，记录“非蓝牙中转输入未释放链路”，不再额外等待。
4. 把默认输入切回 A，并最多等待 `2` 秒确认 A 已恢复。
5. 只在 A 恢复后执行统一恢复观察。C 上的临时恢复不能算成功。

没有可用 C、C 无法成为默认输入或 A 无法恢复时，记录本步骤失败并进入第三步；不得改用蓝牙输入。

## 第三步：同时切换默认输入和输出并切回

本步骤开始时保存当前默认输入为 A、当前默认输出为 B，并按[强制统一约束：选择其他非蓝牙设备](#强制统一约束选择其他非蓝牙设备)分别选择其他非蓝牙输入 C 和其他非蓝牙输出 D。缺少 C 或 D 时不执行半套动作，记录跳过并进入第四步。

固定执行顺序：

1. 把默认输入从 A 切换到 C 并确认成功。
2. 把默认输出从 B 切换到 D 并确认成功。此时输入、输出必须同时处于非蓝牙设备。
3. 先把默认输出切回 B，输入继续保持 C。
4. 最多等待 `500` 毫秒观察目标链路转为 `tacl`；若出现，从出现时刻起继续保持输入 C `1` 秒。未出现时不再额外等待。
5. 最后把默认输入切回 A。
6. 只有 A、B 都恢复后才执行统一恢复观察；C、D 上的临时状态不能算成功。

任一切换失败时，必须按“先恢复输出 B，再恢复输入 A”的顺序恢复所有已经改变的路由，然后进入第四步。不得把其他蓝牙设备作为 C 或 D。
...
处理规则与第一步相同：发送正常退出请求，最多等待 `2` 秒确认退出，然后执行稳定恢复观察。进程不退出或再次提交同类请求时，可以请求一次“仅限本次开机”的阻止自动拉起授权，等待最多 `30` 分钟；授权后仍失败就进入第五步。
...
1. 只切换默认输入，不永久改变默认输出。
2. 中转输入必须按[强制统一约束：选择其他非蓝牙设备](#强制统一约束选择其他非蓝牙设备)选择；没有非蓝牙候选时跳过切换，不得使用其他蓝牙设备。
3. 每次等待候选成为默认输入最多 `2` 秒，每 `100` 毫秒检查一次。
...
第五步只执行一次，不再显示“保留输入还是保留输出”的组合选择，也不因观察失败回到前五步重新匹配。

## 第六步：完整声音链路重建

前五步均未恢复时，只执行一次下列固定序列，不在任意两个节点之间插入恢复观察，也不因中间某项失败而回到前五步：
...
- 重建前的应用处理步骤只能结束已由实时端点、格式请求或多蓝牙拒绝日志明确归属的应用进程。
- `audioaccessoryd`、`audiomxd`、`bluetoothd`、`bluetoothuserd`、`coreaudiod`、系统启动管理器和系统核心进程不得作为普通原因进程结束。
- 第六步只允许按固定顺序和真实服务身份重启这五个服务；蓝牙重新打开后只连接当前目标设备，不得再次先断开目标。
- 任何证据读取失败都必须写入详细日志。证据不足意味着“不结束进程”，不意味着跳过后续安全步骤。
- 每一步成功即停；每一步失败只向后走；禁止回跳、循环复查、重复授权或盲试全部动作。

## 检查清单

- 是否严格执行“实时占用 → 只切换输入并切回 → 同时切换输入输出并切回 → 格式请求 → 不同蓝牙设备与切换复位 → 完整声音链路重建”。
- 格式请求是否已经与实时麦克风占用分开，不再共用一个门槛。
- 所有“切换到其他非蓝牙设备”的步骤是否只使用“内置 ＞ 其他有线或接收器 ＞ 其他非蓝牙”，并彻底排除其他蓝牙设备。
- 第二步是否保存 A、切到非蓝牙 C、按等待机制切回 A，并只验收恢复后的 A。
- 第三步是否保存 A、B，切到非蓝牙 C、D，按“先恢复 B、再恢复 A”的顺序切回，并只验收恢复后的 A、B。
