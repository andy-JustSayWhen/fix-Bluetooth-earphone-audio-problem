...
function main(): void {
  ...
  const historyFile = process.platform === "win32"
    ? join(dirname(fileURLToPath(import.meta.url)), "..", "logs", "bluetooth-audio-history-" + process.pid + ".etl")
    : undefined;
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && url.pathname === "/api/microphone-occupancy/release") {
      try {
        const body = await readLocalJsonBody(request, listeningPort) as { deviceName?: unknown; pids?: unknown; releaseKind?: unknown };
