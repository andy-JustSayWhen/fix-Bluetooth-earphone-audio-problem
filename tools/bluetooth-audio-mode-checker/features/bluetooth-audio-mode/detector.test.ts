...
test("无法形成占用证据的声音活动必须脱离具体设备卡片展示", () => {
  ...
  assert.match(source, /formatRequestOccupancyOverview/);
  assert.match(source, /最后一次格式请求未释放/);
  assert.match(source, /已立即计入麦克风占用/);
  assert.match(source, /user\.confirmedDeviceNames\?\.length/);
});
...
test("双蓝牙抖动时立即刷新但不自动发起修复", () => {
  ...
  assert.doesNotMatch(recoveryClient, /routeChoiceId|route-choice/);
});
