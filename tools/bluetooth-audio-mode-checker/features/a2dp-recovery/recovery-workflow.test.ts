...
function inputOnlyDevice(
  ...
  return device({
    ...
    isDefaultInput: defaultInput === name,
...
test("没有蓝牙麦克风占用时不支持 A2DP 的输入设备仍参与受影响输出的多端点修复", async () => {
  let evidenceReads = 0;
  let microphoneReads = 0;
...
test("解除占用后重新匹配链路残留，观察到tacl后保持中转输入1秒", async () => {
  ...
  assert.ok(
    result.steps.findIndex((step) => step.stage === "重新匹配当前原因") <
      result.steps.findIndex((step) => step.stage === "临时切换输入"),
  );
});
...
test("目标为 tsco 时仍先把未闭合 0 -> 1 计入麦克风占用并结束进程", async () => {
  let running = true;
  let rate = 16_000;
  let mode: AudioModeAssessment["mode"] = "HFP_HSP";
  let terminations = 0;
  ...
  const result = await runRecovery({ name: "蓝牙耳机" }, runtime({
    ...
    readEvidence: () => ({
      ...emptyEvidence,
      events: parseSystemAudioLog(request),
      rawLines: [request],
    }),
    readModeAssessment: () => ({
      name: "蓝牙耳机",
...
test("未闭合请求进程未退出时保持占用状态并进入授权", async () => {
  ...
  assert.equal(result.actionRequired?.kind === "relaunch-authorization"
    ? result.actionRequired.occupancyEvidence
    : null, "unclosed-format-request");
  assert.match(result.actionRequired?.prompt ?? "", /未能退出且仍存在未闭合的 0 -> 1/);
});
...
test("多端点与蓝牙麦克风占用同时存在时先结束占用进程再提供路由选择", async () => {
  let running = true;
  let evidenceReads = 0;
  ...
  assert.equal(result.steps.some((step) => step.stage === "解除已确证原因" && step.status === "成功"), true);
...
test("未闭合格式请求进程二次命中时按麦克风占用升级授权", async () => {
  let terminations = 0;
  const rawLines = [
  ...
  assert.equal(result.actionRequired?.kind === "relaunch-authorization"
    ? result.actionRequired.occupancyEvidence
    : null, "unclosed-format-request");
  assert.match(result.actionRequired?.prompt ?? "", /退出后再次提交未闭合的 0 -> 1/);
  assert.equal(terminations, 0);
});
...
test("链路残留处理已经执行过时直接进入单次重连而不重复切换输入", async () => {
  ...
  assert.equal(reconnects, 1);
  assert.deepEqual(routeChanges, []);
  assert.equal(result.steps.filter((step) => step.stage === "临时切换输入").length, 0);
});
...
test("未闭合 0 -> 1 不受非默认蓝牙设备低采样率影响并计入麦克风占用", async () => {
  let running = true;
  let rate = 16_000;
  const rawLines = [
    "2026-07-19 10:00:00.000000+0800 localhost coreaudiod[1]: [ 30114 ]BTUnifiedAudioDevice: kBluetoothAudioDevicePropertyFormat request 0 ->1",
...
test("证据不足时先切换最高优先级输入再恢复原输入", async () => {
  let defaultInput = "蓝牙耳机";
  let rate = 16_000;
  let reconnects = 0;
  const readDevices = () => [
...
test("中转输入优先选择内置而不是先枚举到的类型不明设备", async () => {
  let defaultInput = "蓝牙耳机";
  let rate = 16_000;
  const routeChanges: string[] = [];
  const readDevices = () => [
    device({ sampleRateOutput: rate, actualSampleRateOutput: rate, isDefaultInput: defaultInput === "蓝牙耳机" }),
    inputOnlyDevice(2, "iPhone 麦克风", "unknown", defaultInput),
    inputOnlyDevice(3, "USB 麦克风", "usb", defaultInput),
    inputOnlyDevice(4, "内置麦克风", "built-in", defaultInput),
    inputOnlyDevice(5, "其他蓝牙麦克风", "bluetooth", defaultInput),
  ];
  ...
  const result = await runRecovery({ name: "蓝牙耳机" }, runtime({
    ...
    readModeAssessment: () => ({
      name: "蓝牙耳机",
      mode: rate > 16_000 ? "A2DP" : "HFP_HSP",
      audioLinkType: defaultInput !== "蓝牙耳机" || rate > 16_000 ? "tacl" : "tsco",
      isDefaultOutput: true,
    } as AudioModeAssessment),
    setDefaultDevice: (direction, candidateName) => {
      if (direction !== "input") return;
      routeChanges.push(candidateName);
      defaultInput = candidateName;
      if (candidateName === "蓝牙耳机") rate = 48_000;
    },
  }));

  assert.equal(result.outcome, "完全恢复");
  assert.deepEqual(routeChanges, ["内置麦克风", "蓝牙耳机"]);
  assert.match(result.steps.find((step) => step.stage === "临时切换输入" && step.status === "成功")?.detail ?? "", /“内置”优先级/);
});

test("内置输入无法切换时先降级到有线而不是类型不明设备", async () => {
  let defaultInput = "蓝牙耳机";
  let rate = 16_000;
  const routeChanges: string[] = [];
  const readDevices = () => [
    device({ sampleRateOutput: rate, actualSampleRateOutput: rate, isDefaultInput: defaultInput === "蓝牙耳机" }),
    inputOnlyDevice(2, "iPhone 麦克风", "unknown", defaultInput),
    inputOnlyDevice(3, "内置麦克风", "built-in", defaultInput),
    inputOnlyDevice(4, "USB 麦克风", "usb", defaultInput),
    inputOnlyDevice(5, "其他蓝牙麦克风", "bluetooth", defaultInput),
  ];
  ...
  const result = await runRecovery({ name: "蓝牙耳机" }, runtime({
    ...
    setDefaultDevice: (direction, candidateName) => {
      if (direction !== "input") return;
      routeChanges.push(candidateName);
      if (candidateName === "内置麦克风") throw new Error("无法切换");
      defaultInput = candidateName;
      if (candidateName === "蓝牙耳机") rate = 48_000;
    },
  }));
  ...
  assert.equal(result.outcome, "完全恢复");
  assert.deepEqual(routeChanges, ["内置麦克风", "USB 麦克风", "蓝牙耳机"]);
  assert.match(result.steps.find((step) => step.stage === "临时切换输入" && step.status === "成功")?.detail ?? "", /“其他有线或接收器”优先级/);
});

test("没有更高优先级中转输入时才使用其他蓝牙设备", async () => {
  let defaultInput = "蓝牙耳机";
  let rate = 16_000;
  const routeChanges: string[] = [];
  const readDevices = () => [
    device({ sampleRateOutput: rate, actualSampleRateOutput: rate, isDefaultInput: defaultInput === "蓝牙耳机" }),
    inputOnlyDevice(2, "其他蓝牙麦克风", "bluetooth", defaultInput),
  ];
  ...
  assert.equal(result.outcome, "完全恢复");
  assert.deepEqual(routeChanges, ["其他蓝牙麦克风", "蓝牙耳机"]);
  assert.match(result.steps.find((step) => step.stage === "临时切换输入" && step.status === "成功")?.detail ?? "", /“其他蓝牙”优先级/);
});
...
test("当前双蓝牙组合复核无未闭合请求后返回组合选择", async () => {
  let evidenceReads = 0;
  const result = await runRecovery({ name: "蓝牙耳机" }, runtime({
    readDevices: () => [
...
test("服务端已确认的多端点选择先复核未闭合请求再执行", async () => {
...
test("等待多端点选择期间出现蓝牙麦克风占用时撤销旧执行并先结束占用进程", async () => {
  let running = true;
  let routeChanges = 0;
  const result = await runRecovery({
    name: "蓝牙耳机",
    ...
    readDevices: () => [
      device({ name: "蓝牙耳机", isDefaultOutput: true }),
      device({
        id: 2,
        name: "蓝牙麦克风",
        uid: "bluetooth-input",
      ...
      inputOnlyDevice(3, "内建设备", "built-in", "蓝牙麦克风"),
    ],
    readMicrophoneUsers: async () => running
      ? [{ ...microphoneUser, devices: ["蓝牙麦克风"] }]
      : [],
    readProcess: () => running ? processInfo : null,
    terminateProcess: () => { running = false; },
    setDefaultDevice: () => { routeChanges += 1; },
  ...
  assert.deepEqual(result.releasedPrograms, ["VoiceApp"]);
  assert.equal(routeChanges, 0);
  assert.match(result.steps.find((step) => step.stage === "复核多端点选择")?.detail ?? "", /先处理/);
});

test("多端点候选层级变化后撤销旧选择并按最新设备重新提供", async () => {
  let routeChanges = 0;
  const result = await runRecovery({
    name: "蓝牙耳机",
    _confirmedRouteChoice: {
    ...
    readDevices: () => [
      device({ name: "蓝牙耳机", isDefaultInput: false, isDefaultOutput: true }),
      device({ name: "蓝牙麦克风", uid: "bluetooth-input", inputChannels: 1, outputChannels: 2, isDefaultInput: true, isDefaultOutput: false }),
      inputOnlyDevice(3, "USB 麦克风", "usb", "蓝牙麦克风"),
      device({ name: "内建设备", uid: "built-in", transport: "built-in", inputChannels: 1, outputChannels: 2, isDefaultInput: false, isDefaultOutput: false }),
    ],
    readModeAssessment: () => ({
      name: "蓝牙耳机",
      mode: "HFP_HSP",
  ...
  assert.equal(result.outcome, "等待选择");
  assert.equal(routeChanges, 0);
  assert.deepEqual(result.actionRequired?.kind === "route-choice"
    ? result.actionRequired.choices.map((choice) => choice.id)
    : [], ["output:内建设备", "input:内建设备"]);
  assert.match(result.steps.find((step) => step.stage === "复核多端点选择")?.detail ?? "", /重新计算/);
});
