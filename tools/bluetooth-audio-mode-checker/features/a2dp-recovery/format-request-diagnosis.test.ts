...
test("请求发生前的 StartIO 不属于请求后的两秒证据窗", () => {
  const earlierStartIo = "2026-07-18 12:47:03.900000+0800 localhost coreaudiod[90589]: BluetoothHALPlugIn_StartIO PID=30114";
  const cause = diagnoseFormatRequestCause(
    evidence(`${earlierStartIo}\n${requestLine}\n${tscoLine}`),
    "Redmi电脑音箱-3899",
    ["Redmi电脑音箱-3899"],
    () => processInfo,
  );
  assert.equal(cause.confidence, "已确认");
  assert.equal(cause.sameProcessStartIo, null);
});
...
test("多端点证据不额外要求目标为低采样率默认输出", () => {
  const cause = diagnoseMultiEndpointCause(
    evidence(multiEndpointLines),
    { ...multiTarget, sampleRateOutput: 48_000, isDefaultOutput: false },
    () => processInfo,
  );
  assert.equal(cause.confidence, "已确认");
});
