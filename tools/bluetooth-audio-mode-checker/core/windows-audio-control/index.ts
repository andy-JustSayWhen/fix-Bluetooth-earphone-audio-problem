...
export function runWindowsControl(action: string, args: string[] = []): Promise<unknown> {
  return new Promise((resolve, reject) => {
    execFile(powershellExecutable(), [...baseArgs, "-Action", action, ...args], options, (error, stdout, stderr) => {
...
export function closeWindowsProcess(process: WindowsProcess): void {
  execFileSync(powershellExecutable(), [...baseArgs, "-Action", "close", "-TargetPid", String(process.pid), "-ExpectedStart", process.startedAt], options);
}
