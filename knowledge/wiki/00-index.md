...
17. [进程映射表](进程映射表.md)
18. [针对链路残留类的一键修复备用方案](针对链路残留类的一键修复备用方案.md)
19. [防止设备进入 HFP 的方法](防止设备进入HFP的方法.md)
20. [耳机处于 A2DP、音频可播放但耳机端没有声音的解决办法](耳机处于A2DP、音频可播放但耳机端没有声音的解决办法.md)
21. [macOS 应用级音频会话日志解析](macOS应用级音频会话日志解析/00-macOS应用级音频会话日志解析.md)
...
- [2026-07-21 QQ音乐指定非默认 K03S 输出的本机证据](../raw/apple/2026-07-21-qqmusic-nondefault-k03s-output-local-evidence.md)
- [2026-07-21 QQ音乐播放、暂停与闲置切换输出的本机证据](../raw/apple/2026-07-21-qqmusic-session-fields-local-evidence.md)
...
- [2026-07-21 QQ音乐播放、暂停与闲置切换输出](macOS应用级音频会话日志解析/cases/2026-07-21-QQ音乐播放暂停与闲置切换输出.md)
...
## 2026-07-21 应用级音频会话字段的冲突检查

- 既有QQ音乐证据确认播放期间会话包含目标 `:output` 端点，本次播放记录再次得到相同结果。
- 本次新增证据补充了暂停后的延迟清理和闲置切换输出无对应系统日志，没有推翻既有结论。
- `deviceUIDs` 被限定为运行状态事件中的端点归属，不再把它扩展解释为应用设置界面持久保存的闲置选择。
- 本轮没有发现需要用户裁决的真实冲突。
