...
| 链路残留类 | 蓝牙输入端点曾被实际占用，受影响设备已经进入 HFP | 结束占用，但保持点击前输入输出组合 | 已确认的占用及结束记录；当前没有新的麦克风读取者；`tsco` 没有及时切回 `tacl`；设备仍为 HFP | 显示链路残留类；先临时切到非蓝牙输入并连续三次确认高采样率，再恢复原输入；只有该动作仍无效才进入其他修复方法；原组合稳定才报告完全恢复 |
