...
| 格式请求类 | 单一低采样率蓝牙输出；没有实际麦克风占用 | 调用微信输入法语音 | `request 0 -> 1`；两秒内 `tsco`；同进程无 `StartIO`；进程号未复用 | 只在完整证据成立时退出请求进程；同一进程第二次命中必须列出进程并等待本次开机阻止重启授权，不得循环退出；不得把普通占用误判为格式请求 |
...
6. 点击一键修复。无论初始迹象指向哪类原因，都先检查全部当前麦克风的实际读取者并尝试一次解除；解除后目标自行恢复就立即结束。目标仍为 HFP 时回到“匹配当前原因”，再进入双蓝牙组合、格式请求、链路残留或兜底重建。页面实时刷新不自动查询历史会话。
7. 记录按钮的阶段提示、每一步动作和最终采样率；修复结束后在 350、900、1800 毫秒再次回读，确认占用没有被应用立即拉起。
8. 恢复测试前输入/输出，并复核没有残留麦克风占用。

前后端续接验收：第一次点击前端只提交目标设备身份并立即显示处理中；后端保存点击现场和本轮处理记录。等待授权或等待组合选择后继续操作时，前端只提交授权或选择，后端继续使用原点击现场、原原因复查次数和原动作记录，不得把续接操作当作新的一键修复。

过时授权验收：麦克风占用类进入等待授权后，制造所列进程在用户点击前自行停止读取的现场。页面收到晚于等待结果的全局占用快照后，必须自动撤销授权并沿用原修复回合继续匹配；若在页面更新前点击授权，服务端也必须实时确认同一进程路径仍属于当前麦克风读取者。占用已经消失时不得启动本次开机阻止任务。进程从未退出时，提示必须写“未能退出且仍在读取”，不得写成“再次读取”或“自动拉起”。

未归属读取验收：让占用检查返回正在读取的进程，但设备名称为空或无法匹配当前蓝牙设备。设备卡必须显示“存在未归属读取”、列出进程，并说明无法确认是否读取本设备；不得显示“未被本机占用”。等待授权是否失效必须根据包含未归属读取者的全局快照判断，不能只看某张设备卡的已归属列表。

防循环验收：同一修复回合的原因复查不得超过四次；同一目标和原路由的链路残留输入切换不得超过一次；同一进程或进程族未经授权只能处理一次、授权后只能再处理一次；目标蓝牙设备重连不得超过一次。达到任一上限后，页面必须显示最终结果或等待用户处理，不得继续显示无终点的修复中状态。
