...
过时授权验收：麦克风占用类进入等待授权后，制造所列进程在用户点击前自行停止读取的现场。页面收到晚于等待结果的占用快照后，必须自动撤销授权并沿用原修复回合继续匹配；若在页面更新前点击授权，服务端也必须实时确认同一进程路径仍属于当前麦克风读取者。占用已经消失时不得启动本次开机阻止任务。进程从未退出时，提示必须写“未能退出且仍在读取”，不得写成“再次读取”或“自动拉起”。
