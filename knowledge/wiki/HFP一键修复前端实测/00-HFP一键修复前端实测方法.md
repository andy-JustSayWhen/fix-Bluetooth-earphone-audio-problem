...
| 链路残留类 | 蓝牙输入端点曾被实际占用，受影响设备已经进入 HFP | 结束占用，但保持点击前输入输出组合 | 已确认的占用及结束记录；当前没有新的麦克风读取者；`tsco` 没有及时切回 `tacl`；设备仍为 HFP | 显示链路残留类；临时切到非蓝牙输入并连续三次确认高采样率后才恢复原输入；原组合稳定才报告完全恢复 |
...
6. 点击一键修复。无论初始迹象指向哪类原因，都先检查全部当前麦克风的实际读取者并尝试一次解除；解除后目标自行恢复就立即结束。只有目标仍为 HFP，才继续当前双蓝牙组合、格式请求、链路残留或兜底重建。页面实时刷新不自动查询历史会话。
...
flowchart TD
    ...
    E -->|否| G{格式请求证据链是否完整}
    G -->|是| H[格式请求类]
    G -->|否| K{本轮已确认并结束蓝牙输入占用且仍为 HFP}
    K -->|是| L[链路残留类]
    K -->|否| J[证据不足，走声音链路重建兜底]
```
