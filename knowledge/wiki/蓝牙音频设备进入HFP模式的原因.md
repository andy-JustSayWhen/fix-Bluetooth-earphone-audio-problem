...
本次设备为 Windows 电脑上的 Bose QC Headphones，与本文 macOS 案例分开记录。用户反馈启用此前被禁用的耳机麦克风后，听感处于降级音频；页面显示输出标称 48 kHz、2 声道，输入标称 16 kHz、1 声道，实际采样率不可读取，模式待确认。

本次读取发现目标存在活动输出，没有检测到目标的活动输入或独立免提输出会话。输入标称格式不能单独证明当前正在使用 HFP；Windows 格式及模式证据的定义见 [Windows 适配规格与设计](../../reference/SPEC/Windows适配.md#模式证据)。微软说明 Windows 11 还会因通信类别的播放请求选择 HFP，现有采集未覆盖该类别，因此没有检测到麦克风会话不足以排除 HFP。依据：[微软传统蓝牙声音说明](https://learn.microsoft.com/en-us/windows-hardware/drivers/bluetooth/bluetooth-classic-audio)。

当前确认的是用户报告的音质降级，以及工具缺少实际链路证据；具体是否使用 HFP、哪个程序触发、是否与启用麦克风直接相关均待核实，不归入已确认原因类别。截图中的重建设备连接操作因管理员权限不足失败，不能记为已完成重建或已恢复音质。

同日核对 main 分支的模式规格，确认模式判定的两组独立证据以 [模式规格判定顺序](../../reference/SPEC/如何判定蓝牙音频设备的音频模式.md#判定顺序) 为准。Windows 分支的独立判定器与恢复执行器均存在绕过共同规则的问题。本轮将规则入口统一、保留独立输出事实、分开显示混音采样率；规则回归通过不代表本机链路采集已经完成。短时系统蓝牙跟踪未捕获可用的模式事件，当前现场仍不能确认具体链路。

同日本机补充读取并在页面核验：输出系统设置为 48 kHz、2 声道，输入为 16 kHz、1 声道；已验证支持的采样率分别为 48 kHz 和 16 kHz。实际运行采样率及当前无线链路仍未取得，因此本次参数读取修复不等于已经确认降级原因或恢复音质。采集来源与边界见 [Windows 适配规格与设计](../../reference/SPEC/Windows适配.md#windows-参数读取)。

本机进一步执行声音端点、驱动节点、直接驱动接口和系统蓝牙事件取证；已取得蓝牙控制器原始事件，尚未捕获能确认目标当前语音链路的事实。节点采样率查询失败不代表耳机或 Windows 无法提供，详见 [运行参数查询记录](../../reference/SPEC/Windows适配.md#本机运行参数查询记录)。

后续蓝牙跟踪已在本机 03:44:47 捕获目标 Bose 的同步语音连接成功事件，原生解析按地址确认该时刻存在语音链路。此证据确认语音模式，仍不能确认具体触发程序、启用麦克风是否为直接原因或实际采样率；详细实现与验证见 [同步语音事件实机取证](../../reference/SPEC/Windows适配.md#同步语音事件实机取证)。

本机 03:54:38 的新实时事件已进入共同判定，页面随后识别 Bose 为 HFP 等低音质语音模式；输入占用仍未被本机检测到，触发原因继续待核实。
