...
后续系统日志补齐了归属边界：2026-07-20 23:47 起，UU远程的 `UURemoteServer` 明确请求屏幕与系统声音采集，`replayd` 随后为该调用创建 44.1 kHz 的 `AudioTap（系统声音抓取通道）`，系统权限日志中的目标进程号也直接指向 `UURemoteServer`。因此，工具把 `replayd` 的无设备归属输入活动显示成“麦克风读取”，不能作为实体麦克风占用证据，更不能据此终止或阻止 `replayd`。该现场应保持为“系统声音采集活动，未证明读取蓝牙麦克风”；完整进程与触发方证据见[进程映射表](进程映射表.md)。
