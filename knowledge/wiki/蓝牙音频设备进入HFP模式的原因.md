...
#### 2026-07-23 Mac mini 点击一键修复导致语音自行结束后的误修复

宿主机、系统和目标设备与上一例相同。用户使用微信输入法语音时，必须先选中文本框再按热键；语音过程中按任意键或鼠标点击其他位置会立即结束语音。用户确认本次先激活语音，看到页面进入 HFP 后立刻点击一键修复；按钮点击使文本框失去焦点，微信输入法语音随即停止，正常情况下设备会自行恢复 A2DP。

本次工具日志确认了点击前后状态变化：

```text
04:50:06.744  微信输入法进程号 10662 提交格式请求 1 ->1。
04:50:06.889  Redmi 链路进入 tsco。
04:50:07.063  工具确认微信输入法正在读取 Redmi 实体麦克风。
04:50:07.823  用户点击一键修复。
04:50:07.975  实时复核已无麦克风占用进程。
04:50:08.190  一键修复把第一步记为“没有已确认占用进程”，但没有观察自行恢复，直接进入输入切换。
04:50:10.014  工具切回 Redmi 默认输入时，仍存活的微信输入法提交新的 0 ->1，目标继续保持 tsco。
04:50:15.009  同时切换输入输出期间出现 1 ->0，目标短暂恢复 tacl。
04:50:16.590  工具再次切回 Redmi 输入时，微信输入法又提交 0 ->1，目标重新进入 tsco。
04:50:23.092  工具最终结束微信输入法进程。
04:50:23.249  目标恢复 tacl；04:50:26.595 连续三秒确认完成。
```

因此，18.815 秒后恢复不代表本次流程合格。直接缺陷是：点击已经结束语音、实时占用也已消失后，流程没有检查设备是否已经自行恢复，反而立即切换并恢复默认输入；恢复 Redmi 输入的动作使仍存活的微信输入法重新提交格式请求，工具随后处理了自己动作期间重新触发的 HFP。修复入口必须在第一步复核无占用进程时保持输入输出不变并立即读取目标模式：当前已经是 A2DP 才连续观察 `3` 秒并在稳定后结束；当前仍为 HFP/HSP 则不等待，直接进入第二步。
