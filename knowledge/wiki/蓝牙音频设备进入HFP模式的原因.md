...
因此，18.815 秒后恢复不代表本次流程合格。直接缺陷是：点击已经结束语音、实时占用也已消失后，流程没有给系统自行恢复的观察窗口，反而立即切换并恢复默认输入；恢复 Redmi 输入的动作使仍存活的微信输入法重新提交格式请求，工具随后处理了自己动作期间重新触发的 HFP。修复入口必须在第一步复核无占用进程时保持输入输出不变，先执行一次稳定恢复观察；目标自行稳定恢复便立即结束，只有未恢复才允许进入输入切换。
