...
1. 进程明确关联实体蓝牙麦克风端点，或同一存活进程最后一次格式请求为 `0 -> 1` 且没有后续 `1 -> 0` 时，即确认麦克风占用，不要求该设备最新链路为 `tsco`。该处理优先于所有后续原因路由：先结束对应进程并观察目标是否稳定恢复 A2DP。未闭合格式请求无法归属实体端点时只显示为页面级占用，不伪造具体设备名称。
