...
| 实例 | 原因归并 | 日志特征 |
| --- | --- | --- |
| C1：Codex 语音输入后进入 HFP；C2：系统设置输入电平监测后进入 HFP | 麦克风占用类 | 进程明确关联实体蓝牙麦克风端点，且该麦克风所属设备最新链路为 `tsco`；停止读取后 `tsco` 断开并恢复 `tacl` 或高采样率 |
| C10：Codex 语音触发 K03S 进入 HFP 后一键修复；C3：微信输入法提交格式请求后进入 HFP | 链路残留类 | 当前没有满足“进程关联实体麦克风端点 + 该麦克风所属设备最新链路为 `tsco`”的已确认占用，但目标设备最新链路仍为 `tsco`；历史占用或格式请求只能解释触发来源，不能覆盖当前残留路由 |
| C7：微信输入法把两个不同蓝牙端点放进同一声音会话后进入 HFP | 多端点会话类 | 当前默认输入和输出来自两台不同蓝牙设备，且目标设备最新链路为 `tsco`；会话中的双蓝牙端点和系统拒绝日志可作为触发来源补充证据 |

多端点会话失败后可能派生格式请求。实际修复先看当前输入输出是否来自两台不同蓝牙设备，再看目标最新链路是否为 `tsco`；两项同时成立才归入多端点会话类。`input_running: true` 只表示存在声音输入活动，不等于已经成功读取实体麦克风。

### 填表说明
一、原因归并标准。归并原因时，以已确认的直接机制为准，不按表面场景命名。

1. 当前默认输入和默认输出来自两台不同蓝牙设备，并且目标设备最新链路为 `tsco`，两项同时成立时，当前修复原因优先归入“多端点会话类”。页面预先显示风险提示不等于完成确诊。
2. 未命中多端点会话类时，仅当进程明确关联实体麦克风端点、且该麦克风所属蓝牙设备最新链路为 `tsco`，才能确认该麦克风被占用并归入“麦克风占用类”。输入和输出是否属于同一台蓝牙设备不影响占用证据本身。
3. 当前没有已确认实体麦克风占用、但目标设备最新链路为 `tsco`，归入“链路残留类”。此前是否抓到过占用进程可以解释触发来源，但不是当前残留类路由的必要条件。
...
原因：链路残留类。当前没有满足“进程明确关联实体麦克风端点 + 该麦克风所属蓝牙设备最新链路为 `tsco`”的已确认占用，但 K03S 最新链路仍为 `tsco`，因此当前修复原因归为链路残留。本例中 Codex (Service) 此前实际占用 K03S 麦克风并触发 HFP；工具在 7 毫秒内确认该进程已经退出，但随后约 10 秒内 K03S 仍未稳定恢复。历史占用解释了 `tsco` 的触发来源，当前无占用而 `tsco` 仍存在则决定残留类路由。
...
原因：当前一键修复原因归为链路残留类。该窗口没有已确认实体麦克风占用，但目标设备已经出现 `Current profile tsco`，因此必须按“无已确认占用 + 当前 `tsco`”进入残留处理。微信输入法提交的蓝牙输入格式请求是本次 `tsco` 的上游触发证据：09:14:36.196 至 09:14:36.200 期间，`audioaccessoryd` 将 Redmi 设为临时默认输入；格式请求进程可反查为微信输入法 `com.tencent.inputmethod.wetype`，同一窗口没有该进程的 `StartIO`。微信输入法为什么产生这条请求仍待确证，且这条历史请求不得覆盖当前残留路由。
...
20:48:19 用户点击一键修复时，工具没有检测到已确认实体麦克风占用，但目标最新链路仍为 `tsco`，所以按当前规则归入链路残留类。修复动作本身曾两次短暂成功：
...
20:48:51.689，微信输入法再次提交 `0 ->1`，随后开始实际读取 Redmi 实体麦克风；20:49:08.495 停止读取，20:49:10.819 设备自行恢复 `tacl`，最终输出为 `44.1 kHz / 2 声道`。这段后续占用是同一进程的新声音输入活动，发生在一键修复已经返回失败之后，不能倒填为 20:48:19 点击现场的占用证据。
...
产品结论：多端点会话类不尝试修改或“修好”具体语音应用，也不把应用身份作为修复门槛。前端发现输入输出来自两台不同蓝牙设备时先提示风险；用户点击一键修复后，服务端只有在该组合仍存在、且目标设备最新链路为 `tsco` 时，才归为多端点会话类并让用户选择保留输入或保留输出。前端继续即时显示模式变化和断连，不自动发起历史日志复核。
...
一键修复按最新现场依次判断：先确认输入输出是否来自两台不同蓝牙设备，并结合目标 `tsco` 决定多端点会话类；未命中时，再用“进程明确关联实体麦克风端点 + 该麦克风所属设备最新链路为 `tsco`”确认占用类；没有已确认占用但目标仍为 `tsco` 时，归为链路残留类。设备列表为空、系统声音采集、虚拟输入和回环输入不得阻挡残留类路由。
...
DJI Mic Mini 作为默认输入运行时，输入端点可以显示 `16 kHz`；这只说明输入端点处于运行状态。只有进程明确关联该实体麦克风端点，并且该设备最新链路为 `tsco`，才能进一步确认麦克风占用。单独的输入采样率不参与设备音频模式判定，也不等于 DJI 的输出音质发生了需要修复的降级。
