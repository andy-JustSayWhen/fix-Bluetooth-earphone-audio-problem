...
| C10：Codex 语音触发 K03S 进入 HFP 后一键修复；C11：LOL 客户端占用 K03S 麦克风后语音结束，残留通话链路使设备停留 HFP | 链路残留类 | 当前既没有进程明确关联实体蓝牙麦克风端点，也没有存活进程留下未闭合 `0 -> 1`，但目标设备最新链路仍为 `tsco` 且没有后续关闭事件；历史占用或已经闭合的格式请求只能解释触发来源，不能覆盖当前残留路由 | 临时把默认输入切换到其他可用设备，观察目标释放通话链路后恢复原输入；若仍未稳定恢复，则断开并重新连接目标蓝牙设备。Windows 侧 C11 另有待验证的第一候选：结束挂在目标输出端点上的维持播放会话 |
...
原因归并：链路残留类（与 C10 同类，Windows 侧实例）。取证时既没有进程关联 K03S 实体蓝牙麦克风端点，通话链路也自 11:34:54 建立后始终没有关闭证据。历史触发来源可归属：LOL 客户端后台服务 `RiotClientServices`（WeGame 拉起的拳头游戏客户端服务进程）在 10:15 至 10:51 间至少 5 次被确认实际读取 K03S 蓝牙麦克风，10:31:41 服务实时捕获到通话链路建立（同窗口也是用户上午的语音实测窗口，见同日上午实例）；11:35:36 至 11:35:53 的微信输入法语音是最后一次麦克风占用。占用全部结束后链路未释放。残留期间 QQ音乐与 LOL 客户端进程持续保有 K03S 输出端点会话，作为下游播放维持者记录；维持作用与最终解除动作均待单变量对照验证。
...
19:4x 独立复核（本机时间）：单次探测确认 K03S 输出端点（A2DP 角色，48 kHz / 2 声道）上唯一活跃会话为 `LeagueClientUxRender(34912)`，QQ音乐进程仍在运行但其 K03S 会话已消失，免提麦克风端点无任何会话；服务跟踪会话 `BluetoothAudioMode-9308` 与 watcher 进程（9308）仍在运行，服务日志自 12:34 起没有任何链路关闭或 A2DP 流恢复记录，用户听感仍为低音质。综合判定：通话链路自 11:34:54 起持续保持打开。
...
flowchart TD
    ...
    F --> G["12:06-12:34<br/>工具探测超时、刷新停止，页面“模式无法确认”"]
    G --> H["19:4x 复核<br/>仅剩 LOL 客户端一个输出会话，链路仍未关闭"]
```

退出 HFP 的方法：待验证。候选顺序：1）结束 LOL 客户端在 K03S 上的播放会话（退出客户端或把其输出切换到其他设备），观察链路是否释放，同时可验证“播放会话维持残留链路”的归因；2）临时把默认输入或默认输出切换到其他设备再恢复；3）断开并重新连接 K03S。本次取证未执行恢复动作，未形成恢复结果。
