...
占用展示和解除规则统一为：同一存活进程最后一次格式请求是未闭合的 `0 ->1` 时，无论是否出现实际声音读取或能否指向具体麦克风，都按“正在占用”处理。能够唯一确定设备时显示在对应设备卡片；不能唯一确定时归入当前默认输入设备卡片，不再单独显示为页面级悬空占用。卡片提交解除后，服务端按同一格式请求证据复核并结束旧进程，不得再次用“是否正在读取实体麦克风”将其过滤。
