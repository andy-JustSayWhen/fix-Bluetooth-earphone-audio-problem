...
macOS 还会为这类麦克风暴露同名输出端点。若该端点不是当前默认输出、没有实际播放，便不能据此推断设备有物理扬声器，更不能要求它恢复到高于 `16 kHz`。模式胶囊统一按该设备输出端点的可用、标称、实际采样率、输出声道和独立 `tacl/tsco` 事实判定；输入活动只追加“麦克风使用中”。产品上的一键修复入口仍只属于已证明从高采样率降级的当前默认输出。案例见 [2026-07-20 占用类前端实测](HFP一键修复前端实测/cases/2026-07-20-占用类前端实测.md)。
