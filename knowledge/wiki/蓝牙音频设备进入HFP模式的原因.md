...
#### 2026-09-16 05:22-05:29 更新后反例与结论撤回

上述“根因已经收敛为旧版 AX200 驱动”的表述在约 5 分钟后即被新证据否定，不能继续作为最终结论。WeGame 于 05:22:36 启动，`pallas.exe` 于 05:23:49 启动；05:24:37 起同步语音链路在“建立、释放”之间多次往返，其中 05:24:37-05:24:43、05:24:46-05:24:53、05:24:56-05:26:03 和 05:27:07-05:27:25 均没有系统确认的麦克风占用。更新后的 `24.10.0.4` 驱动因此没有永久消除“无录音进入 HFP”。此前两轮普通提示音通过，只能证明当时的干净基线没有复现，不能证明驱动是唯一根因。

05:26:22-05:26:34 与 05:26:35-05:26:39 是另一类事件：系统明确记录 `wetype_update.exe` 实际使用 K03S 蓝牙麦克风，约 2 秒后释放同步语音链路，属于微信输入法语音输入触发的正常 HFP。05:27:07 的再次建立发生在微信输入法录音已经结束、占用列表为空之后，不能归因于该次录音仍未释放。

05:27 的端点快照显示 K03S 高音质输出仍向应用公布为 `48 kHz / 2 声道`，蓝牙通话输入端在新驱动下为 `8 kHz / 1 声道`。用户同时听到网页视频音调升高并有倍速感；正常 HFP 只应降低带宽和音质，不应改变播放速度，因此该现象提示高音质与通话链路快速切换期间存在播放时钟或采样率解释错配。当前 `pallas.exe` 同时在 K03S 高音质输出端和蓝牙通话输入端留下暂停会话，而且它的启动早于第一轮无占用 HFP 约 48 秒，是下一步严格停止/重启对照的首要嫌疑对象；现有证据只有时间相关性，尚不能把它写成已确认触发者。05:28:28-05:28:46 的连续只读观察中，链路保持 A2DP、没有麦克风占用或活动会话。

05:31:35 按用户授权只结束 `pallas.exe`，原进程号 65104 约 2 秒内被 WeGame 自动拉起为 55532，说明单次结束不能形成隔离。随后继续只结束新出现的 `pallas.exe` 并观察 45 秒；该进程不再出现，链路全程保持 A2DP、麦克风占用为空。用户在同一观察窗口内主动关闭了 WeGame，结束时 WeGame、`wegame_env.exe`、`pallas.exe` 及英雄联盟相关进程均为零，并确认网页视频听感恢复正常。05:32:41 最终只读复核为 A2DP、`48 kHz / 2 声道`、无麦克风占用。由于关闭 WeGame 与隔离 `pallas.exe` 的时间重叠，本轮能够确认 WeGame 进程组参与异常，却不能在主程序、环境进程和 `pallas.exe` 之间完成最终二分；需要下一轮“保留 WeGame、持续阻止 `pallas.exe`”以及“恢复 `pallas.exe`”的往返测试才能点名具体子进程。

05:50 后用户重新打开 WeGame，初始链路保持 A2DP；`pallas.exe` 于 05:57:54 启动，`RiotClientServices.exe` 于 05:58:03 启动，LOL 客户端于 05:58:07 启动，05:58:32 建立 HFP。该窗口没有麦克风隐私占用；快照显示 `pallas.exe` 和 `RiotClientServices.exe` 均在 K03S 高音质播放端与蓝牙通话输入端留下暂停会话。LOL 界面实际播放会话直到 05:58:55 才开始，晚于 HFP 建立约 23 秒，因此客户端界面声音和网页视频不是本次建立原因。用户在 HFP 建立后立即听到网页视频升调和倍速感。
...
#### 2026-09-16 10:12-10:19 黄色输入活动框漏报复核

用户再次报告 K03S 正处于 HFP，但页面黄色“其他声音输入活动”区域没有显示可能相关的 LOL 语音程序。10:19 只读接口确认：K03S 当前同步语音链路建立于 10:12:56，模式为 HFP；`League of Legends.exe` 正在 K03S 输出端播放，但蓝牙输入端活动标志为否，系统麦克风隐私记录也为空。随后直接枚举全部输入端点会话，K03S 蓝牙麦克风上存在 `Weixin.exe`、`RiotClientServices.exe`、`wetype_update.exe`、`pallas.exe` 和系统会话，状态均为暂停；其中 `pallas.exe` 进程号为 61252，`RiotClientServices.exe` 进程号为 70428。当前证据没有证明这些进程正在录音，也不能仅凭暂停会话重新确认本次 HFP 的唯一原因；但它们属于 HFP 现场应展示的相关声音对象。

漏报原因是 Windows 麦克风使用合成只收集活动输入会话和系统隐私使用记录，明确过滤了暂停会话；黄色区域因此收到空列表。修正规则为：设备当前存在同步语音链路时，把该设备蓝牙输入端上仍存活进程的暂停会话列入黄色区域，但不计入已确认麦克风占用。页面最终说明为：“下面这些程序以前打开过耳机麦克风。它们现在没有录音，但可能没有把耳机的通话模式完全关掉，所以耳机仍停在低音质通话模式。请勾选需要处理的程序并点击‘结束进程’；或者点击‘一键修复’，尝试自动修复。”候选列表显示程序名称、进程号和选择框，使用固定列宽并分别左对齐；列表下方提供统一“结束进程”按钮。该操作复用设备卡片麦克风占用的同一接口与精确结束能力，后端在动作前重新确认每个已选进程仍是对应 HFP 设备输入端上的暂停会话。完整的麦克风路由复位、输入输出共同复位、耳机连接重启及最终蓝牙重建顺序仍由一键修复规格约束。这样既能显示 `pallas.exe` 与 Riot 语音相关进程，又不把相关性冒充因果，也不要求用户理解内部会话术语。

同日使用 ChatGPT 语音输入时，Windows 精确取证到实际声音会话属于一个与主程序同名的后台子进程；主进程与子进程均显示为 `ChatGPT.exe`，只能通过进程号、启动时间和进程参数区分。工具原先虽然锁定了正确子进程，却使用“关闭主窗口”请求结束；后台声音子进程没有主窗口，因此动作失败。修正规则为：前端继续按任务管理器名称原样显示，不为具体应用维护人工别名；后台复核进程号和启动时间后，Windows 直接结束证据锁定的精确进程，不扩大到同名主进程、父进程或进程树，并在动作后重新检查真实麦克风占用。

继续核对服务时间线发现，本次链路并非从早先状态一直残留：10:12:40 同步语音链路曾释放；10:12:57 `chrome.exe` 开始向 K03S 播放，10:12:59 再次建立同步语音链路。由此可以确认 Chrome 播放是这一次重新进入 HFP 的直接时间触发点，却不能把 Chrome 写成底层残留来源；正常高音质播放本不应要求 HFP。蓝牙输入端同时存在 `pallas.exe`、`RiotClientServices.exe`、微信和微信输入法的暂停会话，这四项均列为本次“HFP 残留候选进程”。结合此前“只结束 `pallas.exe` 后约 7 秒释放 HFP”的受控结果，`pallas.exe` 是本机历史证据最强的候选，但本次没有执行逐进程隔离，不能把其他候选从当前因果链中排除。页面因此逐项标注候选，不伪造唯一元凶。
...
### 2026-09-16 Windows 新机 WeGame 子进程自动重启

同一台 `WinAndyWu` 上，XIBERIA K03S 处于 HFP 时，黄色“其他声音输入活动”区域显示 `pallas.exe`。旧进程号 `76436` 的启动时间为 12:23:37，父进程为 `wegame.exe`（进程号 `67288`）。精确结束 `76436` 后，旧身份确实消失，但 WeGame 随即以新进程号 `87852` 重新启动 `pallas.exe`，新进程再次出现在 K03S 的输入会话中。因此“旧进程已退出”和“程序没有重新出现”必须分开报告，不能把自动重启误写成结束失败，也不能只显示普通成功。

本机分别验证了按进程号强制结束和 Windows 进程管理接口结束。第二种接口返回成功值 `0` 后旧身份消失；后续页面读取到输入活动清空，K03S 恢复 A2DP。该轮结果只能证明两条精确结束通道均可工作，不能证明第二条通道天然阻止程序重启。正式规则为：第一种方式报错或旧身份仍存在时，复核同一进程身份后使用第二种方式兜底一次；旧身份消失后若同名程序以新进程号重新建立同一设备的输入活动，只报告“结束成功，但该进程已重启”，不继续结束新进程，也不结束 WeGame 父进程或整个进程树。
