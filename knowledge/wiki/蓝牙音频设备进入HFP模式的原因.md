...
| 实例 | 原因归并 | 日志特征 |
| --- | --- | --- |
| C1：Codex 语音输入后进入 HFP；C2：系统设置输入电平监测后进入 HFP；C3：微信输入法提交格式请求后进入 HFP | 麦克风占用类 | 进程明确关联实体蓝牙麦克风端点，或同一存活进程最后一次格式请求为 `0 -> 1` 且没有后续 `1 -> 0`；不要求该麦克风先出现 `tsco`，停止读取或结束请求进程后观察目标是否恢复 `tacl` 或高采样率 |
| C10：Codex 语音触发 K03S 进入 HFP 后一键修复 | 链路残留类 | 当前既没有进程明确关联实体蓝牙麦克风端点，也没有存活进程留下未闭合 `0 -> 1`，但目标设备最新链路仍为 `tsco`；历史占用或已经闭合的格式请求只能解释触发来源，不能覆盖当前残留路由 |
...
1. 进程明确关联实体蓝牙麦克风端点，或同一存活进程最后一次格式请求为 `0 -> 1` 且没有后续 `1 -> 0` 时，即确认麦克风占用，不要求该设备最新链路为 `tsco`。该处理优先于所有后续原因路由：先结束对应进程并观察目标是否稳定恢复 A2DP。未闭合格式请求前后 `2` 秒内只有一台具有蓝牙输入端点的设备出现地址已归属的 `tsco` 事实时，页面归入该设备卡片；无法唯一匹配时只显示为页面级占用，不伪造具体设备名称。
2. 目标未恢复且当前已无确认蓝牙麦克风占用时，默认输入和默认输出来自两台不同蓝牙设备，并且目标设备最新链路为 `tsco`，两项同时成立才归入“多端点会话类”。页面预先显示风险提示不等于完成确诊。
3. 当前既没有已确认实体蓝牙麦克风读取，也没有存活进程留下未闭合 `0 -> 1`，但目标设备最新链路为 `tsco`，归入“链路残留类”。此前是否抓到过占用进程可以解释触发来源，但不是当前残留类路由的必要条件。
