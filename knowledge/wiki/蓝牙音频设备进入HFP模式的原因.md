...
03:57:33，同步语音链路再次建立；03:57:50 复核时三个微信输入法进程均已重新出现，设备再次处于 HFP，麦克风占用列表仍为空。该对照把本轮归因从单纯时间关联提升为可重复的直接证据：清空微信输入法进程后语音链路关闭，进程自动重启后语音链路重新建立。单次结束进程不能形成持久修复；微信输入法作为当前输入法会被系统再次调用。管理服务已保持停止、开机启动方式未修改，因此重启来源不只依赖该服务。
