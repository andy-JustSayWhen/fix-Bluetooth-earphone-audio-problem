...
2026-07-21 00:21 的现场随后暴露出授权有效期问题：修复回合在 00:21:11.899 仍检测到 `replayd` 读取麦克风并生成等待授权，但 00:21:12.446 的下一次全局占用扫描已经确认没有本机读取者。进程仍存在不等于它仍在读取麦克风。此类授权必须随较新的占用快照失效；前端撤销等待状态并沿用原回合继续匹配，后端在真正执行授权前也必须再次确认同一路径仍属于当前读取者，不能凭旧结果阻止已经停止读取的进程。
