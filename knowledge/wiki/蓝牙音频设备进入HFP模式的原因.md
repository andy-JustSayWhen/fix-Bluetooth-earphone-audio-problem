...
| 实例 | 原因归并 | 日志特征 |
| --- | --- | --- |
| C1：Codex 语音输入后进入 HFP；C2：系统设置输入电平监测后进入 HFP；C10：Codex 语音触发 K03S 进入 HFP 后一键修复 | 麦克风占用类 | 占用状态明确显示本机进程实际读取目标蓝牙麦克风；日志出现输入启动或 `tsco` 输入读取；停止读取后 `tsco` 断开并恢复 `tacl` 或高采样率 |
...
原因：Codex (Service) 实际占用 K03S 麦克风并触发 HFP，属于麦克风占用类。工具按固定第一步先请求 Codex (Service) 正常退出，并在 7 毫秒内确认该进程已经退出；但随后约 10 秒内 K03S 没有稳定恢复，因此本次最终恢复不能归功于“解除 Codex 占用”这一项动作。
